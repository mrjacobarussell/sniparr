# Sniparr Development Guidelines
...
cd /Users/home/Sniparr/Sniparr.io && docker-compose down && COMPOSE_BAKE=true docker-compose up -d --build
...
- **ALWAYS test**: Docker, Windows, Mac, Linux, subpaths (`domain.com/sniparr/`)
...
import sqlite3
conn = sqlite3.connect('/config/sniparr.db')
...
```
**Debug Steps:**
1. Check database exists: `docker exec sniparr ls -la /config/sniparr.db`
2. Verify tables: `docker exec sniparr sqlite3 /config/sniparr.db ".tables"`
3. Check environment detection: Look for `/config` directory existence
4. Test local vs Docker paths: `./data/sniparr.db` vs `/config/sniparr.db`
...
<label><a href="https://sniparr.io/threads/name-field.18/" class="info-icon">
<label><a href="/Sniparr.io/docs/#/configuration" class="info-icon">
<label><a href="#" class="info-icon">

// ✅ FIXED - GitHub documentation with anchors (consolidated 3rd Party Apps page)
<label><a href="https://plexguide.github.io/Sniparr.io/apps/index.html#radarr" class="info-icon">
<label><a href="https://plexguide.github.io/Sniparr.io/apps/index.html#radarr" class="info-icon">
<label><a href="https://plexguide.github.io/Sniparr.io/apps/index.html#swaparr" class="info-icon">
```
**Pattern:** `https://plexguide.github.io/Sniparr.io/apps/index.html#[app-name]`
**Requirements:**
- Always use `https://plexguide.github.io/Sniparr.io/` domain
...
response = requests.get("https://plexguide.github.io/Sniparr.io/manifest.json", timeout=10)
...
### Overview
**Sniparr has migrated from JSON files to SQLite database for all persistent data storage.**

**Database Location:**
- **Docker:** `/config/sniparr.db` (persistent volume)
- **Local Development:** `{project_root}/data/sniparr.db`
...
class DatabaseManager:
    def _get_database_path(self):
        ...
        if os.path.exists('/config'):  # Docker environment
            return '/config/sniparr.db'
        else:  # Local development
            project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
            return os.path.join(project_root, 'data', 'sniparr.db')
...
cd /Users/home/Sniparr/Sniparr.io
python3 -m venv venv
...
# 3. Database will be auto-created at: ./data/sniparr.db
...
docker exec -it sniparr sqlite3 /config/sniparr.db
...
docker exec sniparr ls -la /config/sniparr.db
docker exec sniparr du -h /config/sniparr.db
```
...
### Before Any Changes
- [ ] Check current working directory: `/Users/home/Sniparr/Sniparr.io`
...
- [ ] **For local testing:** `python main.py` (uses ./data/sniparr.db)
- [ ] **For Docker testing:** `docker-compose down && COMPOSE_BAKE=true docker-compose up -d --build`
- [ ] Check logs: `docker logs sniparr`
- [ ] **NEW:** Verify database operations work in both environments
- [ ] Test functionality

### Before Committing
- [ ] Test in Docker environment (database at /config/sniparr.db)
- [ ] Test in local environment (database at ./data/sniparr.db)
- [ ] Test cross-platform compatibility
- [ ] Test subpath scenarios (`domain.com/sniparr/`)
...
grep -r "href.*plexguide.github.io" frontend/ --include="*.js" | grep -v "plexguide.github.io/Sniparr.io" | wc -l | xargs echo "wrong domain links:"
...
### Database & Storage
- `/config/sniparr.db` - **Docker:** Main database file (persistent)
- `./data/sniparr.db` - **Local:** Main database file (development)
...
**Symptoms:** Works on localhost, fails on domain.com/sniparr/
...
### Log Issues
1. Check logs in database: `docker exec sniparr python3 -c "import sys; sys.path.insert(0, '/app/src'); from primary.utils.logs_database import get_logs_database; db = get_logs_database(); logs = db.get_logs(limit=10); [print(f'{log[\"timestamp\"]} - {log[\"app_type\"]} - {log[\"level\"]} - {log[\"message\"]}') for log in logs]"`
...
1. **Check database exists and is accessible:**
   ...
   # Docker environment
   docker exec sniparr ls -la /config/sniparr.db
   docker exec sniparr sqlite3 /config/sniparr.db ".tables"
   
   # Local environment
   ls -la ./data/sniparr.db
   sqlite3 ./data/sniparr.db ".tables"
   ```

2. **Verify table schemas and data:**
   ```bash
   # Check specific table structure
   docker exec sniparr sqlite3 /config/sniparr.db ".schema settings"
   
   # Check data exists
   docker exec sniparr sqlite3 /config/sniparr.db "SELECT COUNT(*) FROM settings;"
   docker exec sniparr sqlite3 /config/sniparr.db "SELECT app_name, COUNT(*) FROM settings GROUP BY app_name;"
   ```
...
4. **Check environment detection:**
   ...
   print(f"Database path would be: {'/config/sniparr.db' if os.path.exists('/config') else './data/sniparr.db'}")
   ```
...
grep -r "href.*plexguide.github.io" frontend/ --include="*.js" | grep -v "plexguide.github.io/Sniparr.io"
```
...
**Database Locations:**
- **Docker:** `/config/sniparr.db` (persistent volume)
- **Local:** `{project_root}/data/sniparr.db`
...
2. **❌ Hard-coded database paths:** `/config/sniparr.db` - Use DatabaseManager auto-detection
...
echo "   Wrong domain links: $(grep -r "href.*plexguide.github.io" frontend/ --include="*.js" | grep -v "plexguide.github.io/Sniparr.io" | wc -l)"
echo ""
...
*Quick reference for Sniparr development. Update when new patterns are discovered.*
