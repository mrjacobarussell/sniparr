"""
Windows System Tray Icon for Sniparr
...
logger = logging.getLogger('SniparrSystemTray')


def _safe_port():
    """Parse port from env with fallback."""
    try:
        return int(os.environ.get("SNIPARR_PORT", os.environ.get("PORT", 9705)))
    except (TypeError, ValueError):
        return 9705


def _find_icon_path():
    """Find the Sniparr icon file.

    Search order (PyInstaller 6.x puts data under _MEIPASS/_internal/):
      1. _MEIPASS/frontend/static/logo/sniparr.ico
      2. _MEIPASS/static/logo/sniparr.ico
      3. _MEIPASS/resources/sniparr.ico
      4. exe_dir/frontend/static/logo/sniparr.ico
    ...
    if getattr(sys, 'frozen', False):
        ...
        candidates += [
            os.path.join(meipass, 'frontend', 'static', 'logo', 'sniparr.ico'),
            os.path.join(meipass, 'static', 'logo', 'sniparr.ico'),
            os.path.join(meipass, 'resources', 'sniparr.ico'),
            os.path.join(exe_dir, 'frontend', 'static', 'logo', 'sniparr.ico'),
        ]
    else:
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..'))
        candidates.append(os.path.join(project_root, 'frontend', 'static', 'logo', 'sniparr.ico'))
...
def _load_icon_image():
    """Load the Sniparr icon as a PIL Image.
    ...
    for logo_dir in logo_dirs:
        for name in ('64.png', '48.png', '32.png', 'sniparr.png'):
...
class SniparrSystemTray:
    """System tray icon for Sniparr on Windows.
    ...
    def _run(self):
        ...
        try:
            ...
            menu = pystray.Menu(
                pystray.MenuItem("Open Sniparr", self._open_web, default=True),
                pystray.Menu.SEPARATOR,
                pystray.MenuItem("Exit Sniparr", self._exit_app),
            )
            self._icon = pystray.Icon("Sniparr", image, "Sniparr", menu)
...
def create_system_tray(port=None, shutdown_callback=None):
    """Factory function. Returns a SniparrSystemTray instance."""
    return SniparrSystemTray(port=port, shutdown_callback=shutdown_callback)
