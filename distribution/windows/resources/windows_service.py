...
try:
    ...
    config_dir = os.path.join(os.environ.get("APPDATA", os.path.expanduser("~")), "Sniparr")
...
def install_service():
    ...
    if not is_admin():
        ...
        print("Alternatively, you can run Sniparr directly without service installation:")
    ...
    try:
        ...
        try:
            ...
            if getattr(sys, 'frozen', False):
                ...
                print("Successfully installed Sniparr as a Windows service using the Python class.")
            ...
            try:
                schService = win32service.OpenService(schSCManager, "Sniparr", win32service.SERVICE_ALL_ACCESS)
                try:
            ...
            print("\nSniparr service installation complete.")
            print("You can now start the service with: net start Sniparr")
            print("Or access Sniparr at: http://localhost:9705")
            
            # Try to start the service automatically
            try:
                win32serviceutil.StartService("Sniparr")
                print("\nSniparr service started successfully.")
                ...
                print("Opening web browser to Sniparr interface...")
            except Exception as start_error:
                print(f"\nNote: Could not automatically start the service: {start_error}")
                print("Please start it manually using: net start Sniparr")
            
            return True
            
        except Exception as install_error:
            print(f"Error installing service: {install_error}")
            print("\nYou can still run Sniparr without the service using:")
            print("  python main.py --no-service")
...
def remove_service(silent=False):
    ...
    if not is_admin():
        if not silent:
            print("ERROR: Administrator privileges required to remove the Sniparr service.")
...
def run_as_cli():
    ...
    try:
        print("Starting Sniparr in CLI mode (non-service)...")
        ...
        print("="*80)
        print(" Sniparr Started Successfully in CLI Mode")
        ...
        print("Sniparr shutdown complete.")
        return 0
