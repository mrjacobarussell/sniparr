#!/usr/bin/env python3
"""
Windows Setup Helper for Sniparr
Assists with configuring Sniparr for Windows environments
"""
...
def setup_environment():
    """Set up Sniparr environment variables and paths"""
    # Determine the base config directory
    app_data = os.environ.get("APPDATA", os.path.expanduser("~"))
    config_dir = os.path.join(app_data, "Sniparr")
    ...
    print(f"Setting up Sniparr configuration in: {config_dir}")
    ...
    try:
        set_environment_variable("SNIPARR_CONFIG_DIR", config_dir)
        print(f"Set SNIPARR_CONFIG_DIR environment variable to: {config_dir}")
    except Exception as e:
...
def check_requirements():
    """Check if all requirements for Sniparr are met"""
    print("Checking Windows requirements for Sniparr...")
    ...
    if python_version.major < 3 or (python_version.major == 3 and python_version.minor < 12):
        print("WARNING: Sniparr requires Python 3.12 or higher")
    ...
    try:
        ...
        s.close()
        if result == 0:
            print("WARNING: Port 9705 is already in use - Sniparr may not start correctly")
            requirements_met = False
        else:
...
def main():
    """Main entry point"""
    print("Sniparr Windows Setup Helper")
    print("===========================")
    ...
    try:
        ...
        if check_requirements():
            ...
            print("Not all requirements are met - Sniparr may not function correctly")
        ...
        if create_test_file(config_dir):
            ...
            print("Sniparr may not function correctly without proper permissions")
        
        print("\nSetup complete!")
        print(f"Sniparr configuration directory: {config_dir}")
        return 0
    except Exception as e:
