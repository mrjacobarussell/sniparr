#!/usr/bin/env python3
"""
Windows path configuration helper for Sniparr
...
def setup_config_directories(base_dir=None):
    """
    Set up and verify all required Sniparr configuration directories
    
    Args:
        base_dir: Optional base directory, if None will use %APPDATA%\Sniparr
    ...
    if base_dir:
        config_dir = base_dir
    elif os.environ.get("SNIPARR_CONFIG_DIR"):
        config_dir = os.environ.get("SNIPARR_CONFIG_DIR")
    else:
    ...
    os.environ["SNIPARR_CONFIG_DIR"] = str(config_path)
...
def verify_service_config():
    ...
    if not success:
        ...
        print("Some features of Sniparr may not work correctly.")
        
        # Try to fall back to a temporary directory if needed
        if not success:
            temp_dir = os.path.join(tempfile.gettempdir(), f"sniparr_config_{os.getpid()}")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print(f"Sniparr config directory: {config_dir}")
    else:
