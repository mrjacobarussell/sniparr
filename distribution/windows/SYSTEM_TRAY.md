...
## Overview
Sniparr now includes a Windows system tray icon that allows it to run silently in the background without cluttering the taskbar.
...
- Uses the Sniparr logo as the icon

### Context Menu Options
Right-click the system tray icon to access:
1. **Open Sniparr** - Opens the web interface in your default browser (default action)
2. **About Sniparr** - Shows information about Sniparr
3. **Exit** - Cleanly shuts down Sniparr

### Double-Click Action
Double-click the system tray icon to quickly open the Sniparr web interface.
...
### How It Works
1. When Sniparr starts on Windows (non-debug mode), it automatically creates a system tray icon
2. The icon runs in a separate background thread
3. The web server and background tasks run normally
4. Users interact with Sniparr through the tray icon menu

### Icon Loading Priority
1. First tries: `frontend/static/logo/sniparr.ico`
...
The `sniparr.spec` file includes:
...
### Before (Old Behavior)
- Console window appears when Sniparr starts
...
### Logging
System tray events are logged to the main Sniparr log:
- `%APPDATA%\Sniparr\logs\sniparr.log`

Look for messages like:
```
[WebServer] Windows system tray icon initialized
[SniparrSystemTray] Starting system tray icon...
[SniparrSystemTray] System tray icon running
```
...
- Sniparr logo files may be missing from the bundle
...
- `distribution/windows/sniparr.spec` - PyInstaller configuration
