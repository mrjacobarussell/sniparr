...
#define MyAppPublisher "Sniparr"
#define MyAppURL "https://github.com/plexguide/Sniparr.io"
#define MyAppExeName "Sniparr.exe"
...
LicenseFile=LICENSE
OutputDir=installer
OutputBaseFilename=Sniparr_Setup
SetupIconFile=frontend\static\logo\sniparr.ico
Compression=lzma
...
Name: "autostart"; Description: "Start Sniparr automatically when Windows starts"; GroupDescription: "Startup options:"; Flags: checkedonce

[Files]
Source: "dist\Sniparr\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs
...
; Launch Sniparr directly after installation
Filename: "{app}\{#MyAppExeName}"; Parameters: "--no-service"; Description: "Start Sniparr"; Flags: nowait postinstall

Filename: "{sys}\cmd.exe"; Parameters: "/c timeout /t 5 && start http://localhost:9705"; Description: "Open Sniparr Web Interface"; Flags: nowait postinstall shellexec
...
[UninstallRun]
; Kill any running instances of Sniparr
...
; Remove the Sniparr startup entry if it exists
...
begin
  ...
  begin
    try
      ...
      Exec(ExpandConstant('{sys}\net.exe'), 'stop Sniparr', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
      Log('Attempted to stop Sniparr service. Result: ' + IntToStr(ResultCode));
  ...
  try
    ...
    begin
      if MsgBox('Sniparr uses port 9705, which appears to be in use. ' +
                'Installation can continue, but Sniparr may not start properly until this port is free. ' +
