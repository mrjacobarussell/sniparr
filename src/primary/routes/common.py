#!/usr/bin/env python3
"""
Common routes blueprint for Sniparr web interface
"""
...
def _cookie_path():
    ...
    When a base URL like '/sniparr' is set, cookies must use that path
...
# --- Login Rate Limiting (delegated to security.rate_limit) --- #
from src.primary.security import rate_limit as _rate_limit
...
def login_route():
    if request.method == 'POST':
        try:
            if not _rate_limit.is_allowed():
                remaining = 0
                logger.warning("Login rate-limited for %s", _rate_limit._real_ip())
                response = jsonify({"success": False, "error": "Too many login attempts. Please wait a moment and try again."})
            ...
            if auth_success:
                _rate_limit.clear_for_client()
                session_token = create_session(username)
                session[SESSION_COOKIE_NAME] = session_token

                from src.primary.security import signed_session
                is_https = request.headers.get('X-Forwarded-Proto') == 'https' or request.is_secure
                signed_value, csrf_token = signed_session.sign_session(session_token)

                redirect_url = (_cookie_path().rstrip('/') or '') + '/'
                response = jsonify({"success": True, "redirect": redirect_url})
                signed_session.set_session_cookies(response, signed_value, csrf_token, secure=is_https)
...
def logout_route():
    try:
        ...
        if session_token:
            logger.info(f"Logging out session token: {session_token[:8]}...")
            username = get_username_from_session(session_token)
            logout(session_token)
            from src.primary.security import audit as _audit
            _audit.log(_audit.LOGOUT, username, request.remote_addr)
        else:
...
def change_password_route():
    ...
    if auth_change_password(current_password, new_password):
        ...
        from src.primary.security import audit as _audit
        _audit.log(_audit.PASSWORD_CHANGED, username, request.remote_addr)
...
def verify_2fa():
    ...
    if verify_2fa_code(username, otp_code, enable_on_verify=True):
        ...
        from src.primary.security import audit as _audit
        _audit.log(_audit.TWO_FA_ENABLED, username, request.remote_addr)
        return jsonify({"success": True})
    else:
...
def disable_2fa_route():
    ...
    if disable_2fa_with_password_and_otp(username, password, otp_code):
        ...
        from src.primary.security import audit as _audit
        _audit.log(_audit.TWO_FA_DISABLED, username, request.remote_addr)
        return jsonify({"success": True})
    else:
...
def shutdown_server():
    """Gracefully shut down Sniparr. Used by the web UI and system tray on Windows."""
    import signal
