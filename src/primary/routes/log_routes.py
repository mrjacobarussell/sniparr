#!/usr/bin/env python3
"""
Database-based log routes for Sniparr web interface
...
def get_logs(app_type):
    ...
    - Main Sniparr Logs page uses app_type=all (includes movie_snipe; user can filter by Movie Hunt in dropdown).
    ...
    try:
        ...
        # When app_type is 'all': include all app types (including movie_snipe)
        if app_type == 'all':
...
def clear_logs(app_type):
    """Clear logs for a specific app type. movie_snipe clear is used by Movie Hunt → Logs; 'all' clears only main apps."""
    try:
        logs_db = get_logs_database()

        # When clearing 'all' (main Logs page): clear only main app logs, never movie_snipe or tv_snipe
        if app_type == 'all':
            deleted_count = logs_db.clear_logs(app_type=None, exclude_app_types=['movie_snipe', 'tv_snipe'])
        else:
            # Map 'system' to actual app type in database
            db_app_type = 'system' if app_type == 'system' else app_type
