...
from ..media_snipe.indexers import (
...
def _dedupe_name(name, db, exclude_id=None):
    ...
    if not name:
        return name
    all_indexers = db.get_indexer_snipe_indexers()
    existing_names = set()
...
@indexer_snipe_bp.route('/api/indexer-snipe/indexers', methods=['GET'])
def api_ih_list():
    """Return all Indexer Hunt indexers (API key masked)."""
    try:
        from src.primary.utils.database import get_database
        db = get_database()
        indexers = db.get_indexer_snipe_indexers()
        out = []
        for idx in indexers:
...
@indexer_snipe_bp.route('/api/indexer-snipe/presets', methods=['GET'])
def api_ih_presets():
...
@indexer_snipe_bp.route('/api/indexer-snipe/indexers', methods=['POST'])
def api_ih_add():
    ...
    try:
        ...
        idx_id = db.add_indexer_snipe_indexer({
            'name': name,
            'display_name': name,
            'preset': preset,
            'protocol': protocol,
...
@indexer_snipe_bp.route('/api/indexer-snipe/indexers/<idx_id>', methods=['PUT'])
def api_ih_update(idx_id):
    """Update an Indexer Hunt indexer."""
    try:
        from src.primary.utils.database import get_database
        db = get_database()
        existing = db.get_indexer_snipe_indexer(idx_id)
        if not existing:
        ...
        db.update_indexer_snipe_indexer(idx_id, updates)
...
@indexer_snipe_bp.route('/api/indexer-snipe/indexers/<idx_id>', methods=['DELETE'])
def api_ih_delete(idx_id):
    ...
    try:
        ...
        db.delete_indexer_snipe_indexer(idx_id)
...
@indexer_snipe_bp.route('/api/indexer-snipe/status', methods=['GET'])
def api_ih_status():
...
@indexer_snipe_bp.route('/api/indexer-snipe/indexers/<idx_id>/test', methods=['POST'])
def api_ih_test(idx_id):
    ...
    try:
        ...
        db = get_database()
        idx = db.get_indexer_snipe_indexer(idx_id)
        if not idx:
...
@indexer_snipe_bp.route('/api/indexer-snipe/validate', methods=['POST'])
def api_ih_validate():
