...
@indexer_snipe_bp.route('/api/indexer-snipe/stats', methods=['GET'])
def api_ih_stats():
    """Aggregated stats across all indexers (rolling 24h window)."""
    try:
        from src.primary.utils.database import get_database
        db = get_database()
        stats = db.get_indexer_snipe_stats_24h()
...
@indexer_snipe_bp.route('/api/indexer-snipe/stats/per-indexer', methods=['GET'])
def api_ih_stats_per_indexer():
    ...
    try:
        ...
        per_indexer = db.get_indexer_snipe_stats_24h_per_indexer()
