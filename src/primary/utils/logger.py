#!/usr/bin/env python3
"""
Logging configuration for Sniparr
...
MAIN_LOG_FILE = LOG_DIR / "sniparr.log"

# App-specific log files (movie_snipe is independent - Activity → Logs only, no *arr)
APP_LOG_FILES = {
...
def setup_main_logger():
    """Set up the main Sniparr logger."""
    global logger
    log_name = "sniparr"
    ...
    log_format = "%(asctime)s - sniparr - %(levelname)s - %(message)s"
...
def get_logger(app_type: str) -> logging.Logger:
    ...
    # Prevent propagation to the main 'sniparr' logger or root logger
    ...
    log_format = f"%(asctime)s - sniparr.{app_type} - %(levelname)s - %(message)s"
    formatter = LocalTimeFormatter(log_format, datefmt="%Y-%m-%d %H:%M:%S")
...
def refresh_timezone_formatters():
    ...
    for app_name, app_logger in app_loggers.items():
        ...
        app_format = f"%(asctime)s - sniparr.{app_type} - %(levelname)s - %(message)s"
