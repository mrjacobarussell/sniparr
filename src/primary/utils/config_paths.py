#!/usr/bin/env python3
"""
Path configuration for Sniparr
...
if not CONFIG_DIR:
    ...
    # Platform-specific fallbacks (secondary options)
        ...
        CONFIG_DIR = os.path.join(os.path.expanduser("~"), "Documents", "Sniparr")
    else:  # Linux (non-Docker)
        CONFIG_DIR = os.path.join(os.path.expanduser("~"), ".config", "sniparr")
...
try:
    ...
    CONFIG_DIR = os.path.join(temp_base, f"sniparr_config_{os.getpid()}")
    CONFIG_PATH = pathlib.Path(CONFIG_DIR)
    CONFIG_PATH.mkdir(parents=True, exist_ok=True)
    print(f"Using temporary config directory: {CONFIG_DIR}")
    
    # Write to Sniparr logs (not Desktop) for visibility in case of issues
    try:
        log_dir = CONFIG_PATH / "logs"
        log_dir.mkdir(exist_ok=True)
        error_log = log_dir / "sniparr_error.log"
