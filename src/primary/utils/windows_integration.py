#!/usr/bin/env python3
"""
Windows Integration for Sniparr
...
def prepare_windows_environment():
    ...
    try:
        ...
        startup_log_path = Path(LOG_DIR) / "sniparr_startup.log"
        try:
        ...
        for directory in [CONFIG_DIR, LOG_DIR]:
            ...
            try:
                ...
                if directory == CONFIG_DIR:
                    fallback_dir = os.path.join(os.path.expanduser("~"), "Documents", "Sniparr")
                    logger.info(f"Attempting to use fallback location: {fallback_dir}")
                    os.makedirs(fallback_dir, exist_ok=True)
                    os.environ["SNIPARR_CONFIG_DIR"] = fallback_dir
