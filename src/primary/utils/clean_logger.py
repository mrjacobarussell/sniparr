...
class CleanLogFormatter(logging.Formatter):
    ...
    def _get_app_type_from_logger_name(self, logger_name: str) -> str:
        ...
        # Handle logger names like "sniparr.sonarr" or just "sniparr"
        if "sniparr" in logger_name.lower():
            parts = logger_name.split(".")
            if len(parts) > 1:
                return parts[-1]  # Return the last part (e.g., "sonarr")
            else:
                return "system"  # Just "sniparr" becomes "system"
        
        # For other logger names, try to extract app type
        known_apps = ["sonarr", "radarr", "lidarr", "readarr", "whisparr", "eros", "sportarr", "swaparr"]
        ...
        try:
            ...
            if not app_type:
                ...
                if hasattr(record, 'name'):
                    if 'sniparr' in record.name.lower():
                        if '.' in record.name:
...
def setup_clean_logging():
    ...
    # Ensure main/sniparr logger also has InstanceLogFilter so logs from get_logger("stats") etc. get APP - INSTANCE
    main_logger = logging.getLogger("sniparr")
