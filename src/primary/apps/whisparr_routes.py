...
def test_connection():
    ...
    try:
        ...
        if version and version.startswith('2'):
            ...
            error_msg = f"Incompatible Whisparr version {version} detected. Sniparr requires Whisparr V2."
            whisparr_logger.error(error_msg)
            return jsonify({"success": False, "message": error_msg}), 400
        else:
            error_msg = f"Unexpected Whisparr version {version} detected. Sniparr requires Whisparr V2."
            whisparr_logger.error(error_msg)
            return jsonify({"success": False, "message": error_msg}), 400
    except ValueError:
            ...
            if response.status_code == 200:
                ...
                if version and version.startswith('2'):
                    ...
                    result = {
                        "name": instance_name,
                        "success": False,
                        "message": f"Incompatible Whisparr version {version} detected. Sniparr requires Whisparr V2.",
                        "version": version
                    }
                else:
                    # Unexpected version
                    result = {
                        "name": instance_name,
                        "success": False,
                        "message": f"Unexpected Whisparr version {version} detected. Sniparr requires Whisparr V2.",
                        "version": version
                    }
            else:
                # API call failed
