...
def arr_request(endpoint: str, method: str = "GET", data: Dict = None, app_type: str = "readarr",
    ...
    headers = {
        "X-Api-Key": key,
        "Content-Type": "application/json",
        "User-Agent": f"Sniparr/1.0 ({app_type})"
    }
...
def get_wanted_missing_books(api_url: str, api_key: str, api_timeout: int, monitored_only: bool = True) -> List[Dict]:
    ...
    # Add User-Agent header to identify Sniparr
    headers = {
        "X-Api-Key": api_key,
...
def tag_processed_author(api_url: str, api_key: str, api_timeout: int, author_id: int, tag_label: str = "sniparr-missing") -> bool:
    """
    ...
    Args:
        ...
        api_timeout: Timeout for the API request
        author_id: The ID of the author to tag
        tag_label: The tag to apply (sniparr-missing, sniparr-upgraded)
