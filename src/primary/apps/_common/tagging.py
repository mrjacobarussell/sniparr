"""
Unified tagging logic for all Sniparr app modules.
...
def extract_tag_settings(app_settings):
    ...
    Returns:
        dict with keys:
            ...
            custom_tags          (dict) – e.g. {"missing": "sniparr-missing", ...}
    """
    return {
...
def get_tag_label(tag_settings, tag_type):
    ...
    if not label:
        label = DEFAULT_TAG_LABELS.get(tag_type, f"sniparr-{tag_type}")
    return label
