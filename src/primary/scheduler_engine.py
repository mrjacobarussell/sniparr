...
# Supported app types
SUPPORTED_APPS = ['sonarr', 'radarr', 'lidarr', 'readarr', 'whisparr', 'eros', 'movie_snipe', 'tv_snipe']
...
def _apply_to_all_apps(action_fn, action_label: str) -> Tuple[bool, str]:
    ...
    for app in SUPPORTED_APPS:
        try:
            if app == 'movie_snipe':
                _apply_to_movie_snipe(None, action_fn, action_label)
            elif app == 'tv_snipe':
                _apply_to_tv_snipe(None, action_fn, action_label)
            else:
...
def _apply_to_app(base_app: str, instance_id: Optional[str], action_fn, action_label: str) -> Tuple[bool, str]:
    ...
    if base_app == 'movie_snipe':
        return _apply_to_movie_snipe(instance_id, action_fn, action_label)
    if base_app == 'tv_snipe':
        return _apply_to_tv_snipe(instance_id, action_fn, action_label)
...
def _apply_to_movie_snipe(instance_id: Optional[str], action_fn, action_label: str) -> Tuple[bool, str]:
    ...
    try:
        db = get_database()
        from src.primary.routes.media_snipe.instances import _get_movie_snipe_instance_settings
        SETTINGS_KEY = "movie_snipe_hunt_settings"

        all_instances = db.get_movie_snipe_instances()
        ...
        if instance_id:
            ...
            settings = _get_movie_snipe_instance_settings(int_id)
            ...
            action_fn(wrapper, 'movie_snipe', None)  # None instance_id since we're already targeting
            ...
            for inst in all_instances:
                ...
                settings = _get_movie_snipe_instance_settings(int_id)
                wrapper = {"enabled": settings.get("enabled", True), "hourly_cap": settings.get("hourly_cap", 20)}
                action_fn(wrapper, 'movie_snipe', None)
                settings["enabled"] = wrapper.get("enabled", settings.get("enabled"))
                settings["hourly_cap"] = wrapper.get("hourly_cap", settings.get("hourly_cap"))
                db.save_app_config_for_instance(SETTINGS_KEY, int_id, settings)
        ...
        return False, f"Error in {action_label} for movie_snipe: {e}"


def _apply_to_tv_snipe(instance_id: Optional[str], action_fn, action_label: str) -> Tuple[bool, str]:
    ...
    try:
        db = get_database()
        from src.primary.routes.media_snipe.instances import _get_tv_snipe_instance_settings
        SETTINGS_KEY = "tv_snipe_hunt_settings"

        all_instances = db.get_tv_snipe_instances()
        ...
        if instance_id:
            ...
            settings = _get_tv_snipe_instance_settings(int_id)
            # Use a wrapper config so the action_fn can modify it
            wrapper = {"enabled": settings.get("enabled", True), "hourly_cap": settings.get("hourly_cap", 20)}
            action_fn(wrapper, 'tv_snipe', None)  # None instance_id since we're already targeting
            settings["enabled"] = wrapper.get("enabled", settings.get("enabled"))
            settings["hourly_cap"] = wrapper.get("hourly_cap", settings.get("hourly_cap"))
            db.save_app_config_for_instance(SETTINGS_KEY, int_id, settings)
            ...
            for inst in all_instances:
                ...
                settings = _get_tv_snipe_instance_settings(int_id)
                wrapper = {"enabled": settings.get("enabled", True), "hourly_cap": settings.get("hourly_cap", 20)}
                action_fn(wrapper, 'tv_snipe', None)
                settings["enabled"] = wrapper.get("enabled", settings.get("enabled"))
                settings["hourly_cap"] = wrapper.get("hourly_cap", settings.get("hourly_cap"))
                db.save_app_config_for_instance(SETTINGS_KEY, int_id, settings)
        ...
        return False, f"Error in {action_label} for tv_snipe: {e}"
...
def load_schedule():
    ...
    try:
        ...
        return {"global": [], "sonarr": [], "radarr": [], "lidarr": [], "readarr": [], "whisparr": [], "eros": [], "sportarr": [], "movie_snipe": [], "tv_snipe": []}
