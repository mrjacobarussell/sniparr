"""
Windows Service module for Sniparr.
Allows Sniparr to run as a Windows service.
...
logger = logging.getLogger('SniparrWindowsService')

class SniparrService(win32serviceutil.ServiceFramework):
    """Windows Service implementation for Sniparr"""
    
    _svc_name_ = "Sniparr"
    _svc_display_name_ = "Sniparr Service"
    ...
    def __init__(self, args):
        ...
        self.sniparr_app = None
        self.stop_flag = None
        
    def SvcStop(self):
        """Stop the service"""
        logger.info('Stopping Sniparr service...')
        self.ReportServiceStatus(win32service.SERVICE_STOP_PENDING)
        win32event.SetEvent(self.stop_event)
        self.is_running = False
        
        # Signal Sniparr to stop properly
        if hasattr(self, 'stop_flag') and self.stop_flag:
            logger.info('Setting stop flag for Sniparr...')
            self.stop_flag.set()
    ...
    def main(self):
        """Main service loop"""
        try:
            logger.info('Starting Sniparr service...')
            
            # Create a service-specific log file
            service_log_file = os.path.join(logs_dir, 'sniparr_service_runtime.log')
            try:
            ...
            try:
                background_thread = threading.Thread(
                    target=start_sniparr, 
                    name="SniparrBackground", 
                    daemon=True
                )
            ...
            logger.info('Sniparr service started successfully')
            servicemanager.LogMsg(
                servicemanager.EVENTLOG_INFORMATION_TYPE,
            ...
            while self.is_running:
                ...
                if not background_thread.is_alive() or not web_thread.is_alive():
                    logger.error("Critical: One of the Sniparr threads has died unexpectedly")
            ...
            logger.info('Sniparr service is shutting down...')
            
            # Set the stop event for Sniparr's background tasks
            if not stop_event.is_set():
                stop_event.set()
            
            # Wait for threads to finish
            logger.info('Waiting for Sniparr threads to finish...')
            background_thread.join(timeout=30)
            web_thread.join(timeout=10)
            
            logger.info('Sniparr service shutdown complete')
            
        except Exception as e:
            logger.exception(f"Critical error in Sniparr service: {e}")
            servicemanager.LogErrorMsg(f"Sniparr service error: {str(e)}")
...
def install_service():
    """Install Sniparr as a Windows service"""
    ...
    if not is_admin():
        ...
        print("Alternatively, you can run Sniparr directly without service installation using '--no-service'.")
        return False
    ...
    try:
        ...
        logger.info("Batch file will remove any existing Sniparr service...")
        with open(log_file, 'a') as f:
            f.write("Will automatically remove any existing Sniparr service\n")
        ...
        try:
            ...
            batch_file = os.path.join(os.environ.get('TEMP', '.'), 'sniparr_service_install.bat')
            with open(batch_file, 'w') as f:
                if getattr(sys, 'frozen', False):
                    # For PyInstaller bundle
                    f.write(f'@echo off\r\n')
                    f.write(f'echo Installing Sniparr service...\r\n')
                    f.write(f'sc stop Sniparr\r\n')  
                    f.write(f'sc delete Sniparr\r\n')
                    f.write(f'sc create Sniparr binPath= "{exe_path} --service" start= auto DisplayName= "Sniparr Service"\r\n')
                    f.write(f'sc description Sniparr "Automated media collection management for Arr apps"\r\n')
                    f.write(f'sc start Sniparr\r\n')
                else:
                    # For Python script
                    f.write(f'@echo off\r\n')
                    f.write(f'echo Installing Sniparr service...\r\n')
                    f.write(f'sc stop Sniparr\r\n')  
                    f.write(f'sc delete Sniparr\r\n')
                    f.write(f'sc create Sniparr binPath= "{sys.executable} \"{script_path}\" --service" start= auto DisplayName= "Sniparr Service"\r\n')
                    f.write(f'sc description Sniparr "Automated media collection management for Arr apps"\r\n')
                    f.write(f'sc start Sniparr\r\n')
            ...
            logger.info("Sniparr service installed successfully")
            
            # Quick check if service is now present
            query_result = subprocess.run('sc query Sniparr', shell=True, capture_output=True, text=True)
            with open(log_file, 'a') as f:
            ...
            print("Sniparr service was created successfully.")
        ...
        try:
            ...
            logger.info("Checking Sniparr service status...")
            with open(log_file, 'a') as f:
            ...
            if is_running:
                print("Sniparr service installed and started successfully!")
                logger.info("Sniparr service is running")
                with open(log_file, 'a') as f:
                ...
                print("2. Try running Sniparr with the 'Run without service' option")
                with open(log_file, 'a') as f:
        ...
        logger.error(f"Error installing Sniparr service: {e}")
        print(f"Error installing Sniparr service: {e}")
        print("Fallback: You can run Sniparr directly using the 'Run without service' option.")
        return False


def remove_service():
    """Remove the Sniparr Windows service"""
    if sys.platform != 'win32':
    ...
    try:
        ...
        try:
            win32serviceutil.StopService("Sniparr")
            print("Sniparr service stopped.")
            time.sleep(2)  # Wait for service to fully stop
        except Exception as e:
            print(f"Note: Could not stop the service (it may already be stopped): {e}")
        
        # Now remove the service
        win32serviceutil.RemoveService("Sniparr")
        print("Sniparr service removed successfully.")
        return True
    except Exception as e:
        print(f"Error removing Sniparr service: {e}")
        return False


def run_as_cli():
    """Run Sniparr as a command-line application (non-service fallback)"""
    try:
        # Import required modules
        import threading
        from primary.background import start_sniparr, stop_event
        from primary.web_server import app
        from waitress import serve
        
        print("Starting Sniparr in command-line mode...")
        ...
        print(f"Error running Sniparr in command-line mode: {e}")
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        if sys.argv[1] == '--install-service' or sys.argv[1] == 'install':
            ...
            servicemanager.PrepareToHostSingle(SniparrService)
