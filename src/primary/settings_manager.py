...
# Known app types
KNOWN_APP_TYPES = ["sonarr", "radarr", "lidarr", "readarr", "whisparr", "eros", "swaparr", "prowlarr", "sportarr", "general"]
...
def load_settings(app_type, use_cache=True):
    ...
    # Apply Lidarr migration (artist -> album) for Sniparr 7.5.0+
    if app_type == "lidarr":
        if current_settings.get("hunt_missing_mode") == "artist":
            settings_logger.info("Migrating Lidarr hunt_missing_mode from 'artist' to 'album' (Sniparr 7.5.0+)")
...
def get_frame_ancestors_setting():
    ...
    if env_val:
        return env_val
    saved = get_advanced_setting("frame_ancestors", "*")
    return (saved or '').strip() or "*"
