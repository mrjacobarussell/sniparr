#!/usr/bin/env python3
"""
Sniparr - Main entry point for the application
...
def app_specific_loop(app_type: str) -> None:
    ...
    app_logger = logging.getLogger(f"sniparr.{app_type}")
                ...
                try:
                    ...
                    if app_type == "sonarr":
                        ...
                        custom_tags = instance_details.get("custom_tags", {
                            "missing": "sniparr-missing",
                            "upgrade": "sniparr-upgrade",
                            "shows_missing": "sniparr-shows-missing"
                        })
...
# ---------------------------------------------------------------------------
# (Media Probe background thread removed — media snipe feature removed)
# ---------------------------------------------------------------------------
...
def start_sniparr():
    """Main entry point for Sniparr background tasks."""
    logger.info(f"--- Starting Sniparr Background Tasks v{__version__} --- ")
    ...
    try:
        ...
        shutdown_threads()
        logger.info("--- Sniparr background tasks stopped --- ")