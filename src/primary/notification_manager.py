#!/usr/bin/env python3
"""
Notification Manager for Sniparr
...
def _send_discord(settings: dict, title: str, message: str, event: str, **kw) -> tuple:
    ...
    username = settings.get("username", "").strip() or "Sniparr"
    ...
    embed = {
        ...
        "description": message,
        "color": color,
        "footer": {"text": "Sniparr Notifications"},
    }
...
def _send_email(settings: dict, title: str, message: str, event: str, **kw) -> tuple:
    ...
    <div style="font-family: -apple-system, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: #1a1a2e; border-radius: 8px; padding: 20px; color: #e0e0e0;">
            ...
            <small style="color: #888;">Sent by Sniparr Notifications</small>
        </div>
    </div>
    """
...
def test_connection(conn_id: int) -> tuple:
    ...
    message = "This is a test notification from Sniparr. If you see this, your notification connection is working correctly! 🎉"
...
def _build_title(base_title: str, conn: dict, extra_data: Optional[dict] = None) -> str:
    ...
    if extra_data:
        if include_app and extra_data.get("app_type"):
            ...
            if app_name == "movie_snipe":
                app_name = "Movie Hunt"
            elif app_name == "tv_snipe":
                app_name = "TV Hunt"
            else:
