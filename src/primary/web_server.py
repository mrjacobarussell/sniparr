...
def get_base_url():
    ...
    This allows Sniparr to run under a subpath like /sniparr when behind a reverse proxy.
    
    Returns:
        str: The configured base URL (e.g., '/sniparr') or empty string if not configured
    """
    try:
...
app.register_blueprint(history_blueprint, url_prefix='/api/sniparr-manager')
...
# Generate a per-request CSP nonce — stored on g so templates can reference {{ g.csp_nonce }}
@app.before_request
...
def _set_response_headers(response):
    ...
    # + TMDB API for Movie Hunt detail fetches
    ...
    response.headers['Content-Security-Policy'] = (
        ...
        "img-src 'self' data: https://image.tmdb.org https://artworks.thetvdb.com https://github.com https://avatars.githubusercontent.com; "
...
def api_settings():
    if request.method == 'GET':
        ...
        # Effective timezone (env TZ overrides settings) for logs, scheduling, Sniparr Manager display
        try:
...
def test_notification():
    ...
    try:
        ...
        success = send_notification(
            title="Sniparr Test Notification",
...
def handle_app_settings(app_name):
    ...
    if request.method == 'GET':
        ...
        # Clean and SSRF-validate URLs before saving
        from src.primary.security.url_validation import validate_instance_url
        if 'instances' in data and isinstance(data['instances'], list):
            for instance in data['instances']:
                if 'api_url' in instance and instance['api_url']:
                    ...
                    err = validate_instance_url(instance['api_url'])
                    if err:
                        return jsonify({"success": False, "error": f"Instance URL rejected: {err}"}), 400
        elif 'api_url' in data and data['api_url']:
            data['api_url'] = data['api_url'].strip().rstrip('/').rstrip('\\')
            err = validate_instance_url(data['api_url'])
            if err:
                return jsonify({"success": False, "error": f"Instance URL rejected: {err}"}), 400
        ...
        if success:
            ...
            from src.primary.security.signed_session import COOKIE_NAME as _SC, unsign_session as _us
            _raw = request.cookies.get(_SC, "")
            _sid = (_us(_raw)[0] if _raw else None) or request.cookies.get(SESSION_COOKIE_NAME, "")
            _actor = get_username_from_session(_sid) if _sid else "unknown"
            _audit.log(_audit.SETTINGS_CHANGED, _actor, request.remote_addr, app_name)
...
def api_security_audit():
    ...
    from src.primary.security.audit import get_recent, purge_old
    purge_old()
    ...
    try:
        ...
        limit = 500
    return jsonify({"success": True, "entries": get_recent(limit=limit, event_filter=event_filter)})
...
def api_health_check():
    ...
    """
    logger = get_logger("system")
    return jsonify({"status": "OK", "message": "Sniparr is running"})
...
def get_github_sponsors():
    ...
    try:
        ...
        if os.path.exists(local_manifest_path):
            ...
            manifest_url = "https://raw.githubusercontent.com/plexguide/Sniparr.io/main/manifest.json"
