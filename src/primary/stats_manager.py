#!/usr/bin/env python3
"""
Statistics Manager for Sniparr
...
def get_default_stats() -> Dict[str, Dict[str, int]]:
    ...
    return {
        ...
        "movie_snipe": {"hunted": 0, "upgraded": 0, "found": 0, "found_upgrade": 0},
        "tv_snipe": {"hunted": 0, "upgraded": 0, "found": 0, "found_upgrade": 0},
        "nzb_snipe": {"completed": 0, "failed": 0}
    }
...
def load_hourly_caps_for_api() -> tuple:
    ...
    try:
        ...
        caps_out = {}
        limits_out = {}
        for app in ["sonarr", "radarr", "lidarr", "readarr", "whisparr", "eros", "sportarr", "movie_snipe", "tv_snipe"]:
            try:
                configured = []
                try:
        ...
        return load_hourly_caps(), {app: _get_app_hourly_cap_limit(app) for app in ["sonarr", "radarr", "lidarr", "readarr", "whisparr", "eros", "sportarr", "movie_snipe", "tv_snipe"]}
