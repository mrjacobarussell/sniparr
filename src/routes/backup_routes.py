"""
Backup and Restore API routes for Sniparr
...
class BackupManager:
    ...
    def _get_backup_directory(self):
        ...
        """
        # Database is already initialized and uses SNIPARR_CONFIG_DIR / Docker / Windows / project data
    ...
    def create_backup(self, backup_type='manual', name=None):
        ...
        try:
            ...
            if not name:
                ...
                name = f"sniparr_backup_v{version}_{timestamp}"
            ...
            for db_name, db_path in databases.items():
                if Path(db_path).exists():
                    ...
                    try:
                        ...
                        logger.warning(f"Could not verify {db_name} before backup: {e}")
    ...
    def _get_all_database_paths(self):
        """Get paths to all Sniparr databases"""
        databases = {}
        
        # Main database
        main_db_path = self.db.db_path
        databases['sniparr'] = str(main_db_path)
    ...
    def restore_backup(self, backup_id):
        ...
        try:
            ...
            timestamp = datetime.now().strftime("%Y.%m.%d_%H.%M.%S")
            current_backup_name = f"sniparr_pre_restore_backup_v{version}_{timestamp}"
