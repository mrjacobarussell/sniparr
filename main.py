...
root_logger = logging.getLogger("SniparrRoot") # Specific logger for this entry point
root_logger.info("--- Sniparr Main Process Starting ---")
    ...
    if sys.argv[1] == '--install-service':
        ...
        try:
            ...
            if sys.argv[1] == '--start':
                ...
                from src.primary.windows_service import SniparrService
                win32serviceutil.HandleCommandLine(SniparrService)
...
try:
    ...
    from primary.background import start_sniparr, stop_event, shutdown_threads
    ...
    sniparr_logger = setup_main_logger()
    ...
    try:
        ...
        sniparr_logger.info("Timezone initialization completed.")
    except Exception as e:
        sniparr_logger.warning(f"Failed to initialize timezone from environment: {e}")
    

    
    # Initialize clean logging for frontend consumption
    setup_clean_logging()
    sniparr_logger.info("Clean logging system initialized for frontend consumption.")
    
    sniparr_logger.info("Successfully imported application components.")
    ...
    try:
        ...
        if os.path.exists(manifest_path):
            ...
            if sponsors_list:
                ...
                sniparr_logger.debug(f"Refreshed {len(sponsors_list)} sponsors from manifest.json")
            else:
                sniparr_logger.warning("No sponsors found in manifest.json")
        else:
            sniparr_logger.warning(f"manifest.json not found at {manifest_path}")
            
    except Exception as e:
        sniparr_logger.error(f"Error refreshing sponsors on startup: {e}")
        raise
    ...
    try:
        ...
        if os.path.exists(version_path):
            ...
            if version:
                ...
                sniparr_logger.info(f"Version {version} loaded into database")
            else:
                sniparr_logger.warning("version.txt is empty")
        else:
            sniparr_logger.warning(f"version.txt not found at {version_path}")
            
    except Exception as e:
        sniparr_logger.error(f"Error loading version to database: {e}")
        # Don't raise - this is not critical enough to stop startup

def run_background_tasks():
    """Runs the Sniparr background processing."""
    bg_logger = get_logger("SniparrBackground") # Use app's logger
    try:
        ...
        bg_logger.info("Starting Sniparr background tasks...")
        start_sniparr() # This function contains the main loop and shutdown logic
    except Exception as e:
        bg_logger.exception(f"Critical error in Sniparr background tasks: {e}")
    finally:
        bg_logger.info("Sniparr background tasks stopped.")
...
def run_web_server():
    ...
    try:
        port = int(os.environ.get('SNIPARR_PORT', os.environ.get('PORT', 9705)))
...
def main_shutdown_handler(signum, frame):
    ...
    sniparr_logger.info(f"Received {signal_name}. Initiating graceful shutdown...")
    ...
    try:
        ...
        sniparr_logger.info("Performing emergency database checkpoint...")
        ...
        try:
            ...
            with main_db.get_connection() as conn:
                ...
                sniparr_logger.info("Main database emergency checkpoint (TRUNCATE) completed")
        except Exception as e:
            sniparr_logger.warning(f"Main database emergency checkpoint failed: {e}")
        ...
        try:
            ...
            with logs_db.get_logs_connection() as conn:
                conn.execute("PRAGMA wal_checkpoint(TRUNCATE)")
                sniparr_logger.info("Logs database emergency checkpoint (TRUNCATE) completed")
        except Exception as e:
            sniparr_logger.warning(f"Logs database emergency checkpoint failed: {e}")
            
    except Exception as e:
        sniparr_logger.warning(f"Emergency database checkpoint failed: {e}")
    ...
    if waitress_server:
        try:
            sniparr_logger.info("Signaling Waitress server to shutdown...")
            waitress_server.close()
        except Exception as e:
            sniparr_logger.warning(f"Error closing Waitress server: {e}")

    # Stop the NZB Sniparr download child process
    try:
        from src.primary.apps.nzb_snipe.download_process import DownloadManagerProxy
        proxy = DownloadManagerProxy._instance
        if proxy is not None:
            sniparr_logger.info("Stopping NZB Sniparr download child process...")
            proxy.stop()
    except Exception as e:
        sniparr_logger.warning(f"Error stopping download child process: {e}")
    ...
    if elapsed_time > shutdown_timeout:
        sniparr_logger.warning(f"Shutdown timeout exceeded ({shutdown_timeout}s). Forcing exit with code 0.")
        os._exit(0)  # Clean exit for Docker updates

def cleanup_handler():
    """Cleanup function called at exit"""
    cleanup_start_time = time.time()
    sniparr_logger.info("Exit cleanup handler called")
    ...
    try:
        ...
        if hasattr(main_db, '_database_instance') and main_db._database_instance is not None:
            sniparr_logger.info("Closing main database connections...")
            ...
            try:
                with main_db.get_connection() as conn:
                    ...
                    sniparr_logger.debug("Main database WAL checkpoint completed")
            except Exception as db_error:
                sniparr_logger.warning(f"Error during main database cleanup: {db_error}")
        ...
        if hasattr(logs_db, '_logs_database_instance') and logs_db._logs_database_instance is not None:
            sniparr_logger.info("Closing logs database connections...")
            try:
                with logs_db.get_logs_connection() as conn:
                    conn.execute("PRAGMA wal_checkpoint(TRUNCATE)")  # Flush WAL to logs database
                    # Skip VACUUM for faster shutdown during updates
                    sniparr_logger.debug("Logs database WAL checkpoint completed")
            except Exception as logs_error:
                sniparr_logger.warning(f"Error during logs database cleanup: {logs_error}")
                
        sniparr_logger.info("Database shutdown completed")
        
    except Exception as e:
        sniparr_logger.warning(f"Error during database shutdown: {e}")
    ...
    sniparr_logger.info(f"Cleanup completed in {cleanup_duration:.2f} seconds")

def main():
    """Main entry point function for Sniparr application.
    ...
    try:
        ...
        sniparr_logger.info(f"Signal handlers not registered (running in background thread): {e}")
    ...
    try:
        ...
        initialize_database()
        sniparr_logger.info("Main database initialization completed successfully")
        ...
        try:
            ...
            sniparr_logger.info("Base URL initialization completed.")
            ...
            sniparr_logger.info("Web server reconfigured with updated base URL.")
        except Exception as e:
            sniparr_logger.warning(f"Failed to initialize base URL from environment: {e}")
        
        # Initialize database logging system (now uses main sniparr.db)
        try:
            from primary.utils.database import get_logs_database, schedule_log_cleanup
            logs_db = get_logs_database()
            schedule_log_cleanup()
            sniparr_logger.info("Database logging system initialized with scheduled cleanup.")
        except Exception as e:
            sniparr_logger.warning(f"Failed to initialize database logging: {e}")
        
        # Load version from version.txt into database on startup
        try:
            load_version_to_database()
        except Exception as version_error:
            sniparr_logger.warning(f"Failed to load version to database: {version_error}")
        
        # Refresh sponsors from manifest.json on startup
        try:
            refresh_sponsors_on_startup()
            sniparr_logger.info("Sponsors database refreshed from manifest.json")
        except Exception as sponsor_error:
            sniparr_logger.warning(f"Failed to refresh sponsors on startup: {sponsor_error}")
        
    except Exception as e:
        sniparr_logger.error(f"Failed to initialize databases: {e}")
        sniparr_logger.error("Application may not function correctly without database")
    ...
    try:
        ...
        background_thread = threading.Thread(target=run_background_tasks, name="SniparrBackground", daemon=True)
        ...
        sniparr_logger.info("KeyboardInterrupt received in main thread. Shutting down...")
        if not stop_event.is_set():
            stop_event.set()
        if not shutdown_requested.is_set():
            shutdown_requested.set()
    except Exception as e:
        sniparr_logger.exception(f"An unexpected error occurred in the main execution block: {e}")
        if not stop_event.is_set():
        ...
        # --- Cleanup ---
        sniparr_logger.info("Web server has stopped. Initiating final shutdown sequence...")

        # Ensure the stop event is set (might already be set by signal handler or error)
        if not stop_event.is_set():
             sniparr_logger.warning("Stop event was not set before final cleanup. Setting now.")
             stop_event.set()

        # Wait for the background thread to finish cleanly
        if background_thread and background_thread.is_alive():
            sniparr_logger.info("Waiting for background tasks to complete...")
            background_thread.join(timeout=5) # Reduced timeout for faster shutdown

            if background_thread.is_alive():
                sniparr_logger.warning("Background thread did not stop gracefully within the timeout.")
        elif background_thread:
             sniparr_logger.info("Background thread already stopped.")
        else:
             sniparr_logger.info("Background thread was not started.")

        # Call the shutdown_threads function from primary.main (if it does more than just join)
        # This might be redundant if start_sniparr handles its own cleanup via stop_event
        # sniparr_logger.info("Calling shutdown_threads()...")
        # shutdown_threads() # Uncomment if primary.main.shutdown_threads() does more cleanup

        sniparr_logger.info("--- Sniparr Main Process Exiting ---")
        
        # Return appropriate exit code based on shutdown reason
        if shutdown_requested.is_set() or stop_event.is_set():
            sniparr_logger.info("Clean shutdown completed - Exit code 0")
            return 0  # Clean shutdown
        else:
            sniparr_logger.warning("Unexpected shutdown - Exit code 1")
