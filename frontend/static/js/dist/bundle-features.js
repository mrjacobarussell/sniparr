...
(function() {
    ...
    var appsModule = {
        ...
        loadAppSettings: function(app) {
            ...
            SniparrUtils.fetchWithTimeout(`./api/settings/${app}`)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(appSettings => {
                    ...
                    if (typeof SettingsForms !== 'undefined') {
                        ...
                        if (window.sniparrUI) {
                            if (!window.sniparrUI.originalSettings) {
                                window.sniparrUI.originalSettings = {};
                            }
                            window.sniparrUI.originalSettings[app] = appSettings;
                        }
        ...
        autoSaveSettings: function(appType, form) {
            ...
            SniparrUtils.fetchWithTimeout(`./api/settings/${appType}`, {
                method: 'POST',
                headers: {
            ...
            .catch(error => {
                ...
                if (typeof sniparrUI !== 'undefined' && typeof sniparrUI.showNotification === 'function') {
                    sniparrUI.showNotification(`Error auto-saving ${appType} settings`, 'error');
                }
            });
        },
        ...
        saveApps: function(event) {
            ...
            if (!appType) {
                ...
                if (typeof sniparrUI !== 'undefined' && typeof sniparrUI.showNotification === 'function') {
                    sniparrUI.showNotification('Error: Could not determine which app settings to save', 'error');
                } else {
                    if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification('Error: Could not determine which app settings to save', 'error');
            ...
            if (!appPanel) {
                ...
                if (typeof sniparrUI !== 'undefined' && typeof sniparrUI.showNotification === 'function') {
                    sniparrUI.showNotification(`Error: App panel not found for ${appType}`, 'error');
                } else {
                    if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification('Error: App panel not found for ' + appType, 'error');
        ...
        saveAppSettings: function(appType, appPanel) {
            ...
            try {
                ...
                if (typeof sniparrUI !== 'undefined' && typeof sniparrUI.showNotification === 'function') {
                    sniparrUI.showNotification(`Error collecting settings: ${error.message}`, 'error');
                } else {
                    if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification('Error collecting settings: ' + error.message, 'error');
            ...
            .catch(error => {
                ...
                if (typeof sniparrUI !== 'undefined' && typeof sniparrUI.showNotification === 'function') {
                    sniparrUI.showNotification(`Error saving settings: ${error.message}`, 'error');
                } else {
                    if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification('Error saving settings: ' + error.message, 'error');
        ...
        markFormAsUnchanged: function(appPanel) {
            ...
            try {
                ...
                if (appType === 'readarr' || appType === 'lidarr' || appType === 'whisparr' || appType === 'whisparrv2') {
                    ...
                    if (window.sniparrUI && window.sniparrUI.formChanged) {
                        window.sniparrUI.formChanged[appType] = false;
                    }
...
window.LogsModule = {
    ...
    // Filter log app dropdown by context: 'media-hunt' = only Media Hunt (All), Movie Snipe, TV Snipe; 'system' = all options
    ...
    // Handle app changes from external sources (like sniparrUI tab switching)
    ...
    loadLogsFromAPI: function(appType, isPolling = false) {
        ...
        SniparrUtils.fetchWithTimeout(`${apiUrl}?${apiParams}`)
            .then(response => {
                return response.json();
            })
            .then(data => {
    ...
    clearLogs: function(fromButton) {
        ...
        const doClear = function() {
            ...
            SniparrUtils.fetchWithTimeout(`./api/logs/${currentApp}/clear`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        .then(data => {
            ...
            if (typeof sniparrUI !== 'undefined' && typeof sniparrUI.showNotification === 'function') {
                sniparrUI.showNotification(`Cleared ${data.deleted_count || 0} ${appLabel}`, 'success');
            }
        ...
        .catch(error => {
            ...
            if (typeof sniparrUI !== 'undefined' && typeof sniparrUI.showNotification === 'function') {
                sniparrUI.showNotification(`Error clearing logs: ${error.message}`, 'error');
            }
        });
        };
        if (window.SniparrConfirm && window.SniparrConfirm.show) {
            window.SniparrConfirm.show({ title: 'Clear Logs', message: msg, confirmLabel: 'Clear', onConfirm: doClear });
        } else {
            if (!confirm(msg)) {
...
window.SniparrLogs = {
    ...
    insertLogInChronologicalOrder: function(newLogEntry) {
        if (!window.sniparrUI || !window.sniparrUI.elements.logsContainer || !newLogEntry) return;
        
        const logsContainer = window.sniparrUI.elements.logsContainer;
    ...
    parseLogTimestamp: function(logEntry) {
        ...
        try {
            ...
            console.warn('[SniparrLogs] Error parsing log timestamp:', error);
            return null;
        }
    },
    
    searchLogs: function() {
        if (!window.sniparrUI || !window.sniparrUI.elements.logsContainer || !window.sniparrUI.elements.logSearchInput) return;
        
        const logsContainer = window.sniparrUI.elements.logsContainer;
        const logSearchInput = window.sniparrUI.elements.logSearchInput;
        ...
        if (window.sniparrUI.elements.clearSearchButton) {
            window.sniparrUI.elements.clearSearchButton.style.display = 'block';
        }
        ...
        if (window.sniparrUI.elements.logSearchResults) {
            window.sniparrUI.elements.logSearchResults.textContent = `Found ${matchCount} matching log entries`;
            window.sniparrUI.elements.logSearchResults.style.display = 'block';
        }
        
        if (window.sniparrUI.elements.autoScrollCheckbox && window.sniparrUI.elements.autoScrollCheckbox.checked) {
            this.autoScrollWasEnabled = true;
            window.sniparrUI.elements.autoScrollCheckbox.checked = false;
        }
    },
    ...
    clearLogSearch: function() {
        if (!window.sniparrUI || !window.sniparrUI.elements.logsContainer) return;
        
        const logsContainer = window.sniparrUI.elements.logsContainer;
        
        if (window.sniparrUI.elements.logSearchInput) {
            window.sniparrUI.elements.logSearchInput.value = '';
        }
        
        if (window.sniparrUI.elements.clearSearchButton) {
            window.sniparrUI.elements.clearSearchButton.style.display = 'none';
        }
        
        if (window.sniparrUI.elements.logSearchResults) {
            window.sniparrUI.elements.logSearchResults.style.display = 'none';
        }
        ...
        if (this.autoScrollWasEnabled && window.sniparrUI.elements.autoScrollCheckbox) {
            window.sniparrUI.elements.autoScrollCheckbox.checked = true;
    ...
    filterLogsByLevel: function(selectedLevel) {
        ...
        if (window.sniparrUI.autoScroll && window.sniparrUI.elements.autoScrollCheckbox && window.sniparrUI.elements.autoScrollCheckbox.checked && visibleCount > 0) {
            setTimeout(() => {
                window.scrollTo({
                    top: 0,
                    behavior: 'smooth'
        ...
        console.log(`[SniparrLogs] Filtered logs by level '${selectedLevel}': showing ${visibleCount}/${totalCount} entries`);
    },
...
window.SniparrInstances = {
    ...
    addAppInstance: function(appName) {
        ...
        if (!container || !window.sniparrUI) return;
        
        const currentSettings = window.sniparrUI.getFormSettings(appName);
        if (!currentSettings.instances) currentSettings.instances = [];
        
        if (currentSettings.instances.length >= 9) {
            if (window.SniparrNotifications) window.SniparrNotifications.showNotification('Maximum of 9 instances allowed', 'error');
            return;
        }
        ...
        if (window.SniparrNotifications) window.SniparrNotifications.showNotification('New instance added', 'success');
    },
    ...
    removeAppInstance: function(appName, instanceId) {
        ...
        if (currentSettings.instances && instanceId >= 0 && instanceId < currentSettings.instances.length) {
            if (currentSettings.instances.length > 1) {
                ...
                if (window.SniparrNotifications) window.SniparrNotifications.showNotification(`Instance "${removedName}" removed`, 'info');
            } else {
                if (window.SniparrNotifications) window.SniparrNotifications.showNotification('Cannot remove the last instance', 'error');
            }
        }
    },
    ...
    testInstanceConnection: function(appName, instanceId, url, apiKey) {
        ...
        const cleanUrl = window.SniparrHelpers ? window.SniparrHelpers.cleanUrlString(url) : url.trim();
        
        SniparrUtils.fetchWithTimeout(`./api/${appName}/test-connection`, {
            method: 'POST',
        ...
        .catch(error => {
            console.error(`[SniparrInstances] Error testing connection:`, error);
...
window.SniparrStateful = {
    loadStatefulInfo: function(attempts = 0, skipCache = false) {
        ...
        console.log(`[SniparrStateful] Loading stateful info (attempt ${attempts + 1}, skipCache: ${skipCache})`);
        ...
        const cachedStatefulData = localStorage.getItem('sniparr-stateful-data');
        if (!skipCache && cachedStatefulData && attempts === 0) {
            try {
                ...
                if (cacheAge < 300000) {
                    console.log('[SniparrStateful] Using cached data while fetching fresh data');
                ...
                console.warn('[SniparrStateful] Failed to parse cached stateful data:', e);
            }
        }

        SniparrUtils.fetchWithTimeout('./api/stateful/info')
            .then(response => {
                if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
                return response.json();
            })
            .then(data => {
                console.log('[SniparrStateful] Received stateful info:', data);
                ...
                localStorage.setItem('sniparr-stateful-data', JSON.stringify({
                    ...data,
                    timestamp: Date.now()
                }));
            })
            .catch(error => {
                console.error('[SniparrStateful] Error loading stateful info:', error);
    ...
    formatDateNicely: function(date) {
        ...
        const userTimezone = window.SniparrHelpers ? window.SniparrHelpers.getUserTimezone() : 'UTC';
    ...
    resetStatefulManagement: function() {
        ...
        .then(data => {
            if (data.success) {
                if (window.SniparrNotifications) window.SniparrNotifications.showNotification('Stateful management reset successfully', 'success');
                setTimeout(() => {
    ...
    updateStatefulExpirationOnUI: function() {
        ...
        .then(data => {
            if (data.success) {
                this.loadStatefulInfo(0, true);
                if (window.SniparrNotifications) window.SniparrNotifications.showNotification(`Updated expiration to ${hours} hours`, 'success');
            } else {
    ...
    updateStatefulExpiration: function(hours) {
        ...
        .catch(error => {
            console.error('[SniparrStateful] Error updating stateful expiration:', error);
        });
    },
    ...
    loadInstanceStateInfo: function(appType, instanceIndex) {
        ...
        SniparrUtils.fetchWithTimeout(`./api/stateful/summary?app_type=${appType}&instance_name=${encodeURIComponent(instanceName)}`, {
            method: 'GET'
        })
        .then(response => response.json())
        .then(summaryData => {
        ...
        .catch(error => {
            console.error(`[SniparrStateful] Error loading state info for ${appType}/${instanceName}:`, error);
    ...
    loadStateManagementForInstance: function(appType, instanceIndex, instanceName) {
        ...
        SniparrUtils.fetchWithTimeout(url, { method: 'GET' })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
        ...
        .catch(error => {
            console.error(`[SniparrStateful] Error loading state management data:`, error);
        });
    },

    refreshTimeDisplays: function() {
        console.log('[SniparrStateful] Refreshing all time displays');
        ...
        if (window.sniparrUI && (window.sniparrUI.currentSection === 'scheduling' || window.sniparrUI.currentSection === 'schedules')) {
            if (typeof loadServerTimezone === 'function') loadServerTimezone();
        }
        
        if (window.sniparrUI && window.sniparrUI.currentSection === 'sniparr-manager' && window.huntManagerModule) {
...
/**
 * Backup and Restore functionality for Sniparr
...
const BackupRestore = {
    ...
    restoreBackup: function() {
        ...
        if (window.SniparrConfirm && window.SniparrConfirm.show) {
            window.SniparrConfirm.show({ title: 'Restore Database', message: 'This will permanently overwrite your current database. Are you absolutely sure?', confirmLabel: 'Restore', onConfirm: doRestore });
        } else {
    ...
    validateDeleteConfirmation: function() {
        ...
        const isValid = confirmationInput.value.toLowerCase() === 'sniparr';
    ...
    deleteDatabase: function() {
        ...
        if (!confirmationInput || confirmationInput.value.toLowerCase() !== 'sniparr') {
            this.showError('Please type "sniparr" to confirm database deletion');
            return;
        }
        ...
        if (window.SniparrConfirm && window.SniparrConfirm.show) {
            window.SniparrConfirm.show({ title: 'Delete Database', message: 'This will PERMANENTLY DELETE your entire Sniparr database. This action CANNOT be undone. Are you absolutely sure?', confirmLabel: 'Delete', onConfirm: doDelete });
        } else {
            if (!confirm('This will PERMANENTLY DELETE your entire Sniparr database. This action CANNOT be undone. Are you absolutely sure?')) return;
            doDelete();
        }
    },
    ...
    deleteBackup: function(backupId) {
        ...
        if (window.SniparrConfirm && window.SniparrConfirm.show) {
            window.SniparrConfirm.show({ title: 'Delete Backup', message: 'Are you sure you want to delete this backup? This action cannot be undone.', confirmLabel: 'Delete', onConfirm: doDelete });
        } else {
    ...
    showSuccess: function(message) {
        if (window.sniparrUI && window.sniparrUI.showNotification) {
            window.sniparrUI.showNotification(message, 'success');
        } else {
            alert(message);
        }
    },
    ...
    showError: function(message) {
        if (window.sniparrUI && window.sniparrUI.showNotification) {
            window.sniparrUI.showNotification(message, 'error');
        } else {
            alert(message);
        }
    },
    ...
    uploadBackup: function() {
        ...
        if (window.SniparrConfirm && window.SniparrConfirm.show) {
            window.SniparrConfirm.show({ title: 'Upload and Restore', message: 'This will permanently overwrite your current database with the uploaded backup. Are you absolutely sure?', confirmLabel: 'Upload', onConfirm: doUpload });
        } else {
...
/**
 * Scheduling functionality for Sniparr
...
window.sniparrSchedules = window.sniparrSchedules || {
    global: [],
    sonarr: [],
    radarr: [],
    lidarr: [],
...
(function() {
    const schedules = window.sniparrSchedules;
    ...
    function setupEventListeners() {
        if (window.sniparrSchedulerInitialized) return;
        ...
        window.sniparrSchedulerInitialized = true;
    }
    ...
    async function loadAppInstances() {
        ...
        try {
            // Fetch standard app settings, Movie Snipe instances, and TV Snipe instances in parallel (cache-bust for fresh data)
            const _ts = Date.now();
            const [settingsResp, movieHuntResp, tvHuntResp] = await Promise.all([
                SniparrUtils.fetchWithTimeout(`./api/settings?t=${_ts}`),
                SniparrUtils.fetchWithTimeout(`./api/movie-snipe/instances?t=${_ts}`).catch(function() { return null; }),
                SniparrUtils.fetchWithTimeout(`./api/tv-snipe/instances?t=${_ts}`).catch(function() { return null; })
            ]);

            if (settingsResp.ok) {
                const settings = await settingsResp.json();
                if (window.sniparrUI) {
                    window.sniparrUI.originalSettings = window.sniparrUI.originalSettings || {};
                    const appTypes = ['sonarr', 'radarr', 'lidarr', 'readarr', 'whisparr', 'eros'];
                    appTypes.forEach(function(at) {
                        if (settings[at]) {
                            window.sniparrUI.originalSettings[at] = window.sniparrUI.originalSettings[at] || {};
                            window.sniparrUI.originalSettings[at].instances = settings[at].instances || [];
                        }
                    });
                }
            }

            // Cache Movie Snipe instances separately (they come from a different API)
            if (movieHuntResp && movieHuntResp.ok) {
                ...
                console.debug('[Scheduler] Movie Snipe instances loaded:', window._movieHuntInstances.length);
            } else {
                window._movieHuntInstances = [];
            }

            // Cache TV Snipe instances separately (they come from a different API)
            if (tvHuntResp && tvHuntResp.ok) {
                ...
                console.debug('[Scheduler] TV Snipe instances loaded:', window._tvHuntInstances.length);
            } else {
    ...
    function loadSchedules() {
        SniparrUtils.fetchWithTimeout('./api/scheduler/load')
    ...
    function saveSchedules() {
        ...
        SniparrUtils.fetchWithTimeout('./api/scheduler/save', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        .then(function(data) {
            if (data.success) {
                if (window.sniparrUI && window.sniparrUI.showNotification) {
                    window.sniparrUI.showNotification('Schedule saved successfully', 'success');
                }
            } else {
                if (window.sniparrUI && window.sniparrUI.showNotification) {
                    window.sniparrUI.showNotification('Failed to save schedule', 'error');
                }
            }
        })
        .catch(function(err) {
    ...
    function addSchedule() {
        ...
        if (isNaN(hour) || isNaN(minute)) {
            if (window.sniparrUI && window.sniparrUI.showNotification) {
                window.sniparrUI.showNotification('Please enter a valid time.', 'error');
...
const historyModule = {
    ...
    init: function() {
        ...
        if (sniparrUI && sniparrUI.currentSection === 'history') {
            this.loadHistory();
        }
    },
    ...
    handleClearHistory: function() {
        ...
        var self = this;
        if (window.SniparrConfirm && window.SniparrConfirm.show) {
            window.SniparrConfirm.show({ title: 'Clear History', message: msg, confirmLabel: 'Clear', onConfirm: function() { self.clearHistory(); } });
        } else {
    ...
    fetchHistoryData: function() {
        ...
        SniparrUtils.fetchWithTimeout(url)
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP error! Status: ${response.status}`);
                }
    ...
    clearHistory: function() {
        ...
        SniparrUtils.fetchWithTimeout(`./api/history/${this.currentApp}`, {
            method: 'DELETE',
        })
            .then(response => {
                if (!response.ok) {
    ...
    // Show error
    showError: function(message) {
        // Use sniparrUI's notification system if available
        if (typeof sniparrUI !== 'undefined' && typeof sniparrUI.showNotification === 'function') {
            sniparrUI.showNotification(message, 'error');
        } else {
            if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification(message, 'error');
            else alert(message);
        }
    },
    ...
    getInstanceSettings: async function(appType, instanceName) {
        ...
        try {
            const response = await SniparrUtils.fetchWithTimeout(`./api/settings/${appType}`);
            if (!response.ok) {
...
// Initialize when sniparrUI is ready
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof sniparrUI !== 'undefined') {
        ...
        const originalSwitchSection = sniparrUI.switchSection;
        
        sniparrUI.switchSection = function(section) {
            // Call original function
            originalSwitchSection.call(sniparrUI, section);
...
(function() {
    ...
    function initUserPage() {
        ...
        // Only set page title when we're actually on the user section
        const isUserSection = (window.location.hash === '#user' || (window.sniparrUI && window.sniparrUI.currentSection === 'user'));
        ...
        localStorage.setItem('sniparr-dark-mode', 'true');
    ...
    function handleUsernameChange() {
        ...
        SniparrUtils.fetchWithTimeout('./api/user/change-username', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                username: newUsername,
    ...
    function handlePasswordChange() {
        ...
        SniparrUtils.fetchWithTimeout('./api/user/change-password', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                current_password: currentPassword,
    ...
    function handleEnableTwoFactor() {
        SniparrUtils.fetchWithTimeout('./api/user/2fa/setup', { method: 'POST' })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                ...
                if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification('Failed to setup 2FA: ' + (data.error || 'Unknown error'), 'error');
                else alert('Failed to setup 2FA: ' + (data.error || 'Unknown error'));
            }
        })
        .catch(error => {
            console.error('Error setting up 2FA:', error);
            if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification('Error setting up 2FA: ' + error.message, 'error');
    ...
    function handleVerifyTwoFactor() {
        ...
        SniparrUtils.fetchWithTimeout('./api/user/2fa/verify', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    function handleDisableTwoFactor() {
        ...
        SniparrUtils.fetchWithTimeout('./api/user/2fa/disable', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                password: password,
    ...
    function handleGenerateRecoveryKey() {
        ...
        SniparrUtils.fetchWithTimeout('./auth/recovery-key/generate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
/**
 * Sniparr - Community Resources Module
...
function initCommunityResourcesVisibilityImmediate() {
    ...
    const sniparrSupportSection = document.querySelector('#sniparr-support-section');
    
    // Try to get cached settings first
    const cachedSettings = localStorage.getItem('sniparr-general-settings');
    if (cachedSettings) {
        try {
            ...
            // Always hide Sniparr Support section
            if (sniparrSupportSection) {
                sniparrSupportSection.style.display = 'none';
            }
...
function initCommunityResourcesVisibility() {
    ...
    // Check if the Sniparr support section exists
    const sniparrSupportSection = document.querySelector('#sniparr-support-section');
    if (!sniparrSupportSection) {
        console.log('[Community] Sniparr support section not found in DOM');
    }
    ...
    SniparrUtils.fetchWithTimeout('./api/settings/general')
        ...
        .then(data => {
            ...
            localStorage.setItem('sniparr-general-settings', JSON.stringify(data));
            ...
            // Always hide Sniparr Support section
            if (sniparrSupportSection) {
                console.log('[Community] Hiding Sniparr support section (always hidden)');
                sniparrSupportSection.style.display = 'none';
            }
        })
        .catch(error => {
...
const GithubSponsors = {
    ...
    getCachedSponsors: function() {
        const cachedData = localStorage.getItem('sniparr-github-sponsors');
    ...
    cacheSponsors: function(sponsors) {
        ...
        localStorage.setItem('sniparr-github-sponsors', JSON.stringify(data));
...
window.SniparrProwlarr = {
    ...
    loadProwlarrStatus: function() {
        ...
        SniparrUtils.fetchWithTimeout('./api/prowlarr/status')
            .then(response => response.json())
            .then(statusData => {
                ...
                if (statusData.configured && statusData.enabled) {
                    ...
                    console.log('[SniparrProwlarr] Prowlarr card hidden - configured:', statusData.configured, 'enabled:', statusData.enabled);
                }
            })
            .catch(error => {
    ...
    loadProwlarrIndexers: function() {
        SniparrUtils.fetchWithTimeout('./api/prowlarr/indexers')
            .then(response => response.json())
            .then(data => {
    ...
    loadProwlarrStats: function() {
        SniparrUtils.fetchWithTimeout('./api/prowlarr/stats')
            .then(response => response.json())
            .then(data => {
    ...
    initializeProwlarr: function() {
        console.log('[SniparrProwlarr] Initializing Prowlarr section');
