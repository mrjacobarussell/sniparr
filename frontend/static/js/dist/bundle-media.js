...
/**
 * MediaUtils — Shared utility functions for media cards across Movie Snipe and Requestarr.
...
(function() {
    ...
    function hideMedia(options) {
        ...
        if (!instanceName) {
            if (window.sniparrUI && window.sniparrUI.showNotification) {
                window.sniparrUI.showNotification('No instance selected.', 'error');
            }
            return;
        }
        ...
        var doHide = function() {
            ...
            .then(function() {
                ...
                if (window.sniparrUI && window.sniparrUI.showNotification) {
                    window.sniparrUI.showNotification('"' + title + '" hidden.', 'success');
                }
            ...
            .catch(function(err) {
                ...
                if (window.sniparrUI && window.sniparrUI.showNotification) {
                    window.sniparrUI.showNotification('Failed to hide media.', 'error');
                }
            });
        };
     ...
     *   - movieHuntInstances {Array|null} - Fallback Movie Snipe instances
    ...
    function resolveMovieInstance(options) {
        ...
        // Try DOM select element (Movie Snipe pattern)
     ...
     *
     * Both Movie Snipe detail and Requestarr detail call setupDetailRefreshListeners()
 ...
 * Uses ./api/movie-snipe/ or ./api/tv-snipe/ (instances, instances/current) based on mode.
 * Exposes MovieHuntInstanceDropdown and TVHuntInstanceDropdown as wrappers for compatibility.
 */
(function() {
    'use strict';

    var baseUrl = (typeof window !== 'undefined' && window.SNIPARR_BASE_URL) ? window.SNIPARR_BASE_URL.replace(/\/$/, '') : '';
    ...
    function getEmptyLabel(mode) {
        return mode === 'tv' ? 'No TV Snipe instances' : 'No Movie Snipe instances';
    }
    ...
    // --- Combined Activity dropdown (Movie Snipe + TV Snipe instances in one select) ---
    ...
    function populateActivityCombined(select, preferMode) {
        ...
        Promise.all([
            ...
            if (movieList.length === 0 && tvList.length === 0) {
                var emptyOpt = document.createElement('option');
                emptyOpt.value = '';
                emptyOpt.textContent = 'No Movie Snipe or TV Snipe instances';
                select.appendChild(emptyOpt);
                select.value = '';
        ...
        select.addEventListener('change', function() {
            ...
            fetch(api(apiBase + '/instances/current'), {
                ...
                .then(function(data) {
                    ...
                    var isTvActivity = /^tv-snipe-activity-(queue|history|blocklist)$/.test(hash);

                    if (mode === 'tv' && isMovieActivity) {
                        var tab = (hash === 'activity-logs') ? 'queue' : hash.replace('activity-', '');
                        window.location.hash = 'tv-snipe-activity-' + tab;
                    } else if (mode === 'movie' && isTvActivity) {
                        var match = hash.match(/^tv-snipe-activity-(queue|history|blocklist)$/);
...
/**
 * Media Hunt – unified discover for Movie Snipe and TV Snipe.
 * Mode from window._mediaHuntSectionMode ('movie' | 'tv'). Uses #media-snipe-* elements.
 */
(function() {
    'use strict';
    ...
    window.MediaHunt = {
        ...
        init() {
            ...
            const mode = getMode();
            const betaEl = document.getElementById('media-snipe-beta-text');
            const searchInput = document.getElementById('media-snipe-search-input');
            const loadingText = document.getElementById('media-snipe-grid-loading-text');
            const filterBtn = document.getElementById('media-snipe-filter-btn');
            const sortSelect = document.getElementById('media-snipe-sort');

            if (betaEl) {
                betaEl.innerHTML = mode === 'movie'
                    ? '<strong>Beta feature:</strong> Movie Snipe is in active development. Things may be broken and will change quickly. There is little to no support until it is officially released. Only USENET (SABnzbd, NZBGet) is supported for now. <a href="https://plexguide.github.io/Sniparr.io/apps/movie-hunt.html#docker" target="_blank" rel="noopener">Wiki &amp; Docker setup <i class="fas fa-external-link-alt" style="font-size: 0.85em;"></i></a>'
                    : '<strong>Beta feature:</strong> TV Snipe is in active development. Things may be broken and will change quickly. Only USENET (SABnzbd, NZBGet) is supported for now.';
            }
            ...
            if (mode === 'movie' && window.MovieHuntInstanceDropdown && document.getElementById('media-snipe-instance-select')) {
                if (!this._instanceSelectReady) {
                    window.MovieHuntInstanceDropdown.attach('media-snipe-instance-select', () => {
                        this.page = 1;
                        this.hasMore = true;
                        this.loading = false;
                        this.requestToken++;
                        const grid = document.getElementById('media-snipe-media-grid');
                ...
                this.requestToken++;
                this.loadMovies(1);
            } else if (mode === 'tv' && window.TVHuntInstanceDropdown && document.getElementById('media-snipe-instance-select')) {
                if (!this._instanceSelectReady) {
                    window.TVHuntInstanceDropdown.attach('media-snipe-instance-select', () => {
                        this.page = 1;
                        this.hasMore = true;
                        this.loading = false;
                        this.loadDiscover();
        ...
        setupSearch() {
            const self = this;
            const input = document.getElementById('media-snipe-search-input');
            if (!input) return;
        ...
        performSearch(query) {
            const mode = getMode();
            const resultsView = document.getElementById('media-snipe-search-results-view');
            const mainView = document.getElementById('media-snipe-main-view');
            const grid = document.getElementById('media-snipe-search-results-grid');
            ...
            if (mode === 'movie') {
                ...
                fetch('./api/tv-snipe/search?q=' + encodeURIComponent(query))
                    .then(function(r) { return r.json(); })
                    .then(function(data) {
                        const results = data.results || [];
                        grid.innerHTML = '';
        ...
        setupFilterButton() {
            const btn = document.getElementById('media-snipe-filter-btn');
            if (btn) {
        ...
        loadMovies(page) {
            ...
            let url = './api/movie-snipe/discover/movies?page=' + page + '&_=' + Date.now();
            ...
            fetch(url)
                ...
                .finally(function() {
                    ...
                    const sentinel = document.getElementById('media-snipe-scroll-sentinel');
        ...
        loadDiscover() {
            ...
            const self = this;
            const sortParam = (document.getElementById('media-snipe-sort') && document.getElementById('media-snipe-sort').value) || 'popularity.desc';
            fetch('./api/tv-snipe/discover/tv?page=' + this.page + '&sort_by=' + encodeURIComponent(sortParam))
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    const results = data.results || [];
        ...
        getSelectedInstanceName() {
            const select = document.getElementById('media-snipe-instance-select');
        ...
        createCard(item) {
            ...
            const instanceSelect = document.getElementById('media-snipe-instance-select');
            ...
            const openRequestModal = function() {
                ...
                if (id && window.RequestarrDiscover && window.RequestarrDiscover.modal) {
                    ...
                    const instSelect = document.getElementById('media-snipe-instance-select');
        ...
        addToCollection(show, instanceIdFromContext) {
            const tvCollectionSelect = document.getElementById('media-snipe-collection-tv-instance-select');
            const collectionSelect = document.getElementById('media-snipe-collection-instance-select'); // legacy fallback
            const discoverSelect = document.getElementById('media-snipe-instance-select');
            ...
            if (!instId) {
                if (window.sniparrUI) window.sniparrUI.showNotification('Please select an instance first.', 'error');
                return;
            }
            ...
            fetch('./api/tv-snipe/collection?instance_id=' + instId, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    tmdb_id: tmdbId,
                ...
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    if (data.exists && window.sniparrUI) window.sniparrUI.showNotification('Series already in collection.', 'info');
                    else if (data.success && window.sniparrUI) window.sniparrUI.showNotification('Added to collection!', 'success');
                    else if (window.sniparrUI) window.sniparrUI.showNotification(data.error || 'Failed to add.', 'error');
                })
                .catch(function() {
                    if (window.sniparrUI) window.sniparrUI.showNotification('Network error adding to collection.', 'error');
                });
        },
        ...
        createShowCard(show) {
            ...
            card.innerHTML = '<div class="media-poster">' +
                '<img src="' + posterUrl + '" alt="' + (typeof SniparrUtils !== 'undefined' ? SniparrUtils.escapeHtml(title) : title.replace(/</g, '&lt;').replace(/>/g, '&gt;')) + '" loading="lazy">' +
                '<div class="media-overlay"><button class="add-to-collection-btn" title="Add to Collection"><i class="fas fa-plus"></i></button></div></div>' +
                '<div class="media-info"><div class="media-title">' + (typeof SniparrUtils !== 'undefined' ? SniparrUtils.escapeHtml(title) : title.replace(/</g, '&lt;').replace(/>/g, '&gt;')) + '</div>' +
 ...
 * Uses media-snipe-* element IDs; calls window.MediaHunt.loadMovies(1) on apply.
...
(function() {
    ...
    function populateGenresSelect() {
        const list = el('media-snipe-genre-list');
        if (!list) return;
        list.innerHTML = '';
        genres.forEach(function(genre) {
            ...
            item.addEventListener('click', function() {
                ...
                autoApplyFilters();
                const dropdown = el('media-snipe-genre-dropdown');
    ...
    function renderSelectedGenres() {
        const container = el('media-snipe-selected-genres');
        if (!container) return;
        container.innerHTML = '';
        ...
        activeFilters.genres.forEach(function(genreId) {
            ...
            remove.addEventListener('click', function(e) {
                ...
                updateModalFilterCount();
                autoApplyFilters();
                const items = document.querySelectorAll('#media-snipe-genre-list .genre-item');
    ...
    function updateSliderRange(type, minInput, maxInput) {
        const rangeEl = el('media-snipe-' + type + '-range');
        if (!rangeEl) return;
    ...
    function updateYearDisplay() {
        const minInput = el('media-snipe-filter-year-min');
        const maxInput = el('media-snipe-filter-year-max');
        if (!minInput || !maxInput) return;
        let min = parseInt(minInput.value, 10);
        let max = parseInt(maxInput.value, 10);
        if (min > max) { var t = min; min = max; max = t; }
        const display = el('media-snipe-year-display');
        if (display) display.textContent = 'Movies from ' + min + ' to ' + max;
    }
    function updateRuntimeDisplay() {
        const minInput = el('media-snipe-filter-runtime-min');
        const maxInput = el('media-snipe-filter-runtime-max');
        if (!minInput || !maxInput) return;
        let min = parseInt(minInput.value, 10);
        let max = parseInt(maxInput.value, 10);
        if (min > max) { var t = min; min = max; max = t; }
        const display = el('media-snipe-runtime-display');
        if (display) display.textContent = min + '-' + max + ' minute runtime';
    }
    function updateRatingDisplay() {
        const minInput = el('media-snipe-filter-rating-min');
        const maxInput = el('media-snipe-filter-rating-max');
        if (!minInput || !maxInput) return;
        let min = parseFloat(minInput.value);
        let max = parseFloat(maxInput.value);
        if (min > max) { var t = min; min = max; max = t; }
        const display = el('media-snipe-rating-display');
        if (display) display.textContent = 'Ratings between ' + min.toFixed(1) + ' and ' + max.toFixed(1);
    }
    function updateVotesDisplay() {
        const minInput = el('media-snipe-filter-votes-min');
        const maxInput = el('media-snipe-filter-votes-max');
        if (!minInput || !maxInput) return;
        let min = parseInt(minInput.value, 10);
        let max = parseInt(maxInput.value, 10);
        if (min > max) { var t = min; min = max; max = t; }
        const display = el('media-snipe-votes-display');
    ...
    function updateFilterDisplay() {
        ...
        const countEl = el('media-snipe-filter-count');
    ...
    function updateModalFilterCount() {
        ...
        if (activeFilters.votesMin > 0 || activeFilters.votesMax < 10000) count++;
        if (activeFilters.hideAvailable) count++;
        const countEl = el('media-snipe-filter-active-count');
        const text = count === 0 ? '0 Active Filters' : count === 1 ? '1 Active Filter' : count + ' Active Filters';
        if (countEl) countEl.textContent = text;
    }

    function loadFilterValues() {
        const yearMin = el('media-snipe-filter-year-min');
        const yearMax = el('media-snipe-filter-year-max');
        const runtimeMin = el('media-snipe-filter-runtime-min');
        const runtimeMax = el('media-snipe-filter-runtime-max');
        const ratingMin = el('media-snipe-filter-rating-min');
        const ratingMax = el('media-snipe-filter-rating-max');
        const votesMin = el('media-snipe-filter-votes-min');
        const votesMax = el('media-snipe-filter-votes-max');
        const hideAvailable = el('media-snipe-hide-available-movies');
        ...
        renderSelectedGenres();
        var items = document.querySelectorAll('#media-snipe-genre-list .genre-item');
    ...
    function autoApplyFilters() {
        var yearMinEl = el('media-snipe-filter-year-min');
        var yearMaxEl = el('media-snipe-filter-year-max');
        var runtimeMinEl = el('media-snipe-filter-runtime-min');
        var runtimeMaxEl = el('media-snipe-filter-runtime-max');
        var ratingMinEl = el('media-snipe-filter-rating-min');
        var ratingMaxEl = el('media-snipe-filter-rating-max');
        var votesMinEl = el('media-snipe-filter-votes-min');
        var votesMaxEl = el('media-snipe-filter-votes-max');
    ...
    function openFiltersModal() {
        var modal = el('media-snipe-filter-modal');
        if (!modal) return;
    ...
    function clearFilters() {
        ...
        var sortSelect = el('media-snipe-sort');
    ...
    function setupEventListeners() {
        var backdrop = el('media-snipe-filter-backdrop');
        var closeBtn = el('media-snipe-filter-close');
        ...
        var hideAvailable = el('media-snipe-hide-available-movies');
        if (hideAvailable) {
        ...
        var genreInput = el('media-snipe-genre-search-input');
        var genreDropdown = el('media-snipe-genre-dropdown');
        ...
        function bindRange(type, updateDisplayFn) {
            var minInput = el('media-snipe-filter-' + type + '-min');
            var maxInput = el('media-snipe-filter-' + type + '-max');
            if (!minInput || !maxInput) return;
        ...
        var yearMin = el('media-snipe-filter-year-min');
        var yearMax = el('media-snipe-filter-year-max');
        if (yearMin && yearMax) {
            yearMin.max = maxYear;
...
/**
 * Media Hunt Activity – Part 1: Movie Snipe (Queue, History, Blocklist, Logs).
...
(function() {
    ...
    var searchQuery = '';
    var isLoading = false;
    // Movie Snipe Logs (independent of main Sniparr logs)
    var logPage = 1;
    var logTotalPages = 1;
    var logTotalLogs = 0;
    var logPageSize = 20;
        ...
        var doClear = function() {
            ...
            .then(function(data) {
                if (data.success) {
                    ...
                    logTotalPages = 1;
                    loadMovieHuntLogs();
                    if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification('Movie Snipe logs cleared.', 'success');
                } else if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification(data.error || 'Failed to clear logs', 'error');
            })
            .catch(function() {
                if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification('Connection error', 'error');
            });
        };
        if (window.SniparrConfirm && window.SniparrConfirm.show) {
            window.SniparrConfirm.show({ title: 'Clear Movie Snipe Logs', message: 'Clear all Movie Snipe logs? This cannot be undone.', confirmLabel: 'Clear', onConfirm: doClear });
        } else {
            if (!window.confirm('Clear all Movie Snipe logs? This cannot be undone.')) return;
            doClear();
        }
    }
    ...
    function loadData() {
        ...
        if (instanceId == null) {
            showEmptyState(true, 'Select an instance', 'Choose a Movie Snipe or TV Snipe instance to view queue, history, or blocklist.');
            return;
        }
        isLoading = true;
        showLoading(true);
    ...
    function removeBlocklistEntry(sourceTitle) {
        ...
        var instanceId = getInstanceId();
        if (instanceId == null) {
            if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification('Select an instance first.', 'warning');
            return;
        }
            ...
            fetch('./api/activity/blocklist', {
                ...
                .then(function(data) {
                    if (data.success !== false) {
                        loadData();
                        if (window.sniparrUI && window.sniparrUI.showNotification) {
                            window.sniparrUI.showNotification('Removed from blocklist.', 'success');
                        }
                    } else if (window.sniparrUI && window.sniparrUI.showNotification) {
                        window.sniparrUI.showNotification(data.error || 'Failed to remove.', 'error');
                    }
                })
                .catch(function() {
                    if (window.sniparrUI && window.sniparrUI.showNotification) {
                        window.sniparrUI.showNotification('Failed to remove from blocklist.', 'error');
                    }
                });
        };
        if (window.SniparrConfirm && window.SniparrConfirm.show) {
            window.SniparrConfirm.show({ title: 'Remove from Blocklist', message: msg, confirmLabel: 'Remove', onConfirm: doRemove });
        } else {
    ...
    function refreshData() {
        ...
        loadData();
        if (window.sniparrUI && window.sniparrUI.showNotification) {
            window.sniparrUI.showNotification('Data refreshed.', 'success');
        }
    }
    ...
    function removeSelected() {
        ...
        if (!checkboxes || checkboxes.length === 0) {
            if (window.sniparrUI && window.sniparrUI.showNotification) {
                window.sniparrUI.showNotification('No items selected.', 'warning');
            }
            return;
        }
        var instanceId = getInstanceId();
        ...
        fetch('./api/activity/queue', {
            ...
            .then(function(data) {
                if (data.success !== false) {
                    loadData();
                    if (window.sniparrUI && window.sniparrUI.showNotification) {
                        window.sniparrUI.showNotification('Selected items removed from queue.', 'success');
                    }
                } else if (window.sniparrUI && window.sniparrUI.showNotification) {
                    window.sniparrUI.showNotification(data.error || 'Failed to remove.', 'error');
                }
            ...
            .catch(function() {
                if (window.sniparrUI && window.sniparrUI.showNotification) {
                    window.sniparrUI.showNotification('Failed to remove selected.', 'error');
                }
            });
    }
    ...
    function init(view) {
        ...
        // Movie Snipe Logs view bindings
...
/**
 * Media Hunt Activity – Part 2: TV Snipe (Queue, History, Blocklist).
 * Exposes window.TVHuntActivityModule. Uses tvHuntActivity* DOM IDs and /api/tv-snipe/.
 */
(function() {
    'use strict';
    ...
    function loadData() {
        ...
        var url = './api/tv-snipe/' + endpoint + '?' + params.toString();
    ...
    function createRow(item) {
        ...
        if (currentView === 'queue') {
            var canSelect = item.id != null && item.id !== '';
            var cb = canSelect
                ? '<td class="col-select"><input type="checkbox" class="tv-snipe-activity-queue-row-cb" data-id="' + escapeHtml(String(item.id)) + '"></td>'
                : '<td class="col-select"></td>';
            ...
            tr.innerHTML =
                ...
                '<td class="col-date">' + dateText + '</td>' +
                '<td class="col-actions">' +
                '<button type="button" class="tv-snipe-activity-blocklist-btn-info" title="Details" data-source-title="' + sourceTitleEsc + '" data-date="' + escapeAttr(item.added_at || '') + '" aria-label="Details"><i class="fas fa-info-circle"></i></button>' +
                '<button type="button" class="tv-snipe-activity-blocklist-btn-remove" title="Remove from blocklist" data-id="' + escapeAttr(id) + '" aria-label="Remove from blocklist"><i class="fas fa-times" style="color: #ef4444;"></i></button>' +
                '</td>';
            var infoBtn = tr.querySelector('.tv-snipe-activity-blocklist-btn-info');
            var removeBtn = tr.querySelector('.tv-snipe-activity-blocklist-btn-remove');
            if (infoBtn) infoBtn.addEventListener('click', function() { showBlocklistDetailsModal(this); });
    ...
    function removeBlocklistEntry(itemId) {
        ...
        var msg = 'Remove this from the TV Snipe blocklist?';
        var doRemove = function() {
            var params = new URLSearchParams({ instance_id: String(instanceId) });
            fetch('./api/tv-snipe/blocklist/' + encodeURIComponent(itemId) + '?' + params.toString(), { method: 'DELETE' })
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    if (data.success !== false) {
                        loadData();
                        if (window.sniparrUI && window.sniparrUI.showNotification) window.sniparrUI.showNotification('Removed from blocklist.', 'success');
                    } else if (window.sniparrUI && window.sniparrUI.showNotification) {
                        window.sniparrUI.showNotification(data.error || 'Failed to remove.', 'error');
                    }
                })
    ...
    function removeSelected() {
        var checkboxes = document.querySelectorAll('#tvHuntActivityQueueTableBody .tv-snipe-activity-queue-row-cb:checked');
        if (!checkboxes || checkboxes.length === 0) {
            if (window.sniparrUI && window.sniparrUI.showNotification) {
                window.sniparrUI.showNotification('No items selected.', 'warning');
            }
        ...
        if (window.sniparrUI && window.sniparrUI.showNotification) {
            window.sniparrUI.showNotification('Bulk remove from queue is not yet implemented for TV Snipe.', 'info');
        }
    }
    ...
    function init(view) {
        ...
        if (selectAllCb) {
            selectAllCb.checked = false;
            selectAllCb.onclick = function() {
                var rowCbs = document.querySelectorAll('#tvHuntActivityQueueTableBody .tv-snipe-activity-queue-row-cb');
                for (var i = 0; i < rowCbs.length; i++) rowCbs[i].checked = selectAllCb.checked;
            };
        }
        var blocklistModal = el('tvHuntActivityBlocklistDetailsModal');
        if (blocklistModal) {
            var closeBtns = blocklistModal.querySelectorAll('.tv-snipe-activity-blocklist-modal-close, .activity-blocklist-modal-close-btn');
            for (var i = 0; i < closeBtns.length; i++) closeBtns[i].addEventListener('click', closeBlocklistDetailsModal);
            blocklistModal.addEventListener('click', function(e) { if (e.target === blocklistModal) closeBlocklistDetailsModal(); });
        }
        var prevBtn = el('tvHuntActivityPrevPage');
 ...
 * Used by: requestarr-content.js, media-hunt.js, media-snipe-collection.js,
...
(function() {
     ...
     * @param {Object} options
     *   - instanceName {string} - Movie Snipe instance display name
                ...
                '<div class="mh-modal">' +
                    ...
                    '<div class="mh-modal-body">' +
                        ...
                        '<label class="mh-check-row">' +
                            ...
                            '<div class="mh-help">Remove this movie from your Movie Snipe collection</div></div>' +
                        '</label>' +
    ...
    function handleConfirm(item, options, hasFile, appType, closeModal) {
        ...
        if (removeFromLib) {
            var removePromise = fetch('./api/movie-snipe/collection/remove?instance_id=' + encodeURIComponent(instanceId), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    tmdb_id: tmdbId,
        ...
        Promise.all(promises)
            .then(function() {
                closeModal();
                if (window.sniparrUI && window.sniparrUI.showNotification) {
                    window.sniparrUI.showNotification('"' + (item.title || 'Movie') + '" removed.', 'success');
                }
            ...
            .catch(function(err) {
                ...
                if (window.sniparrUI && window.sniparrUI.showNotification) {
                    window.sniparrUI.showNotification('Delete failed: ' + (err.message || 'Unknown error'), 'error');
                }
...
/**
 * Movie Snipe Movie Collection - requested movies and status (requested / available).
...
(function() {
    ...
    window.MovieHuntCollection = {
        _prefix: 'media-snipe-movie-collection',
        page: 1,
        ...
        getEl: function(suffix) {
            return document.getElementById((this._prefix || 'media-snipe-collection') + '-' + suffix);
        },

        init: function() {
            this.page = 1;
            this.viewMode = SniparrUtils.getUIPreference(this._prefix + '-view', 'posters');
        ...
        performSearch: function(query) {
            ...
            // Use the currently selected Movie Snipe instance for library status check
        ...
        setupInstanceSelect: function() {
            ...
            var select = this.getEl('instance-select');
            if (!select) return;
            var id = (this._prefix || 'media-snipe-collection') + '-instance-select';
            if (window.MovieHuntInstanceDropdown && window.MovieHuntInstanceDropdown.attach) {
                ...
                select.innerHTML = '<option value="">No Movie Snipe instances</option>';
            }
        },
        ...
        setupSort: function() {
            ...
            var saved = SniparrUtils.getUIPreference('movie-snipe-collection-sort', 'title.asc');
            if (saved) {
                self.sortBy = saved;
            ...
            select.onchange = function() {
                ...
                SniparrUtils.setUIPreference('movie-snipe-collection-sort', self.sortBy);
                self.page = 1;
                self.loadCollection();
            };
        },
        ...
        setupViewMode: function() {
            ...
            select.onchange = function() {
                ...
                SniparrUtils.setUIPreference(self._prefix + '-view', self.viewMode);
                self.renderPage();
            };
        },
        ...
        loadCollection: function() {
            ...
            var url = './api/movie-snipe/collection?page=' + this.page + '&page_size=' + this.pageSize + '&sort=' + encodeURIComponent(this.sortBy || 'title.asc');
        ...
        renderPage: function() {
            ...
            var noInstances = !val || (opt && (opt.value === '' || (opt.textContent || '').trim().indexOf('No Movie Snipe') !== -1));
            ...
            if (noInstances) {
                if (grid) {
                    ...
                    grid.innerHTML = '<div style="text-align: center; color: #9ca3af; max-width: 600px;">' +
                        ...
                        '<p style="font-size: 20px; margin-bottom: 15px; font-weight: 500;">No Movie Snipe instance</p>' +
                        '<p style="font-size: 15px; line-height: 1.6; opacity: 0.8; margin-bottom: 20px;">Create a Movie Snipe instance to manage your media collection and requested movies.</p>' +
                        '<a href="./#media-snipe-instances" class="action-button" style="display: inline-flex; align-items: center; gap: 8px; background: rgba(99, 102, 241, 0.2); border: 1px solid rgba(99, 102, 241, 0.4); color: #818cf8; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 500; transition: all 0.2s ease;">' +
                        '<i class="fas fa-cog"></i> Set up Movie Snipe instance</a></div>';
                }
                return;
            }
        ...
        removeFromCollection: function(title, year) {
            ...
            if (!title) return;
            var doRemove = function() {
                fetch('./api/movie-snipe/collection/0', {
                    method: 'DELETE',
                    headers: { 'Content-Type': 'application/json' },
                    ...
                    .then(function(data) {
                        if (data.success && window.sniparrUI && window.sniparrUI.showNotification) {
                            window.sniparrUI.showNotification('Removed from collection.', 'success');
                        }
                        self.loadCollection();
                    })
                    .catch(function() {
                        if (window.sniparrUI && window.sniparrUI.showNotification) {
                            window.sniparrUI.showNotification('Failed to remove.', 'error');
                        }
                    });
            };
            if (window.SniparrConfirm && window.SniparrConfirm.show) {
                window.SniparrConfirm.show({ title: 'Remove from Requested List', message: 'Remove this movie from your requested list?', confirmLabel: 'Remove', onConfirm: doRemove });
            } else {
                doRemove();
            }
        }
...
/**
 * TV Snipe Collection – series list with expandable seasons and episodes.
...
(function() {
    ...
    window.TVHuntCollection = {
        _prefix: 'media-snipe-tv-collection',
        items: [],
        sortBy: 'title.asc',
        viewMode: 'posters',
        searchQuery: '',
        ...
        setupInstanceSelect: function() {
            ...
            fetch('./api/tv-snipe/instances')
                ...
                .then(function(data) {
                    ...
                    fetch('./api/tv-snipe/instances/current')
                        .then(function(r) { return r.json(); })
                        .then(function(d) {
        ...
        performCollectionSearch: function(query) {
            ...
            fetch(url)
                ...
                .catch(function(err) {
                    if (grid) grid.innerHTML = '<p style="text-align:center;color:#f87171;">' + (err && err.message ? SniparrUtils.escapeHtml(err.message) : 'Search failed.') + '</p>';
                });
        },
        ...
        _createSearchCard: function(show) {
            ...
            card.innerHTML =
                '<div class="media-poster">' +
                    '<img src="' + posterUrl + '" alt="' + SniparrUtils.escapeHtml(title) + '" loading="lazy">' +
                ...
                '<div class="media-info">' +
                    '<div class="media-title">' + SniparrUtils.escapeHtml(title) + '</div>' +
        ...
        loadCollection: function() {
            ...
            fetch('./api/tv-snipe/collection?instance_id=' + instanceId)
                .then(function(r) { return r.json(); })
                .then(function(data) {
        ...
        renderCollection: function() {
            ...
            if (noInstances) {
                if (grid) {
                    ...
                    grid.innerHTML = '<div style="text-align: center; color: #9ca3af; max-width: 600px;">' +
                        '<i class="fas fa-cube" style="font-size: 64px; margin-bottom: 30px; opacity: 0.4; display: block;"></i>' +
                        '<p style="font-size: 20px; margin-bottom: 15px; font-weight: 500;">No TV Snipe instance</p>' +
                        '<p style="font-size: 15px; line-height: 1.6; opacity: 0.8; margin-bottom: 20px;">Create a TV Snipe instance to manage your TV collection and requested shows.</p>' +
                        '<a href="./#media-snipe-instances" class="action-button" style="display: inline-flex; align-items: center; gap: 8px; background: rgba(99, 102, 241, 0.2); border: 1px solid rgba(99, 102, 241, 0.4); color: #818cf8; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 500; transition: all 0.2s ease;">' +
                        '<i class="fas fa-cog"></i> Set up TV Snipe instance</a></div>';
                }
                return;
            }
            ...
            if (items.length === 0) {
                if (grid) {
                    ...
                    grid.innerHTML = '<div style="text-align: center; color: #9ca3af; max-width: 600px;">' +
                        '<i class="fas fa-inbox" style="font-size: 64px; margin-bottom: 30px; opacity: 0.4; display: block;"></i>' +
                        '<p style="font-size: 20px; margin-bottom: 15px; font-weight: 500;">No Requested Media</p>' +
                        '<p style="font-size: 15px; line-height: 1.6; opacity: 0.8;">TV shows you add from TV Snipe will appear here. Track status as Requested or Available.</p></div>';
                }
                return;
            }
        ...
        _renderTableToContainer: function(tableEl, tbody, items) {
            ...
            items.forEach(function(series) {
                ...
                tr.innerHTML =
                    ...
                    '<td class="table-title">' + SniparrUtils.escapeHtml(series.title || '') + '</td>' +
                    '<td>' + (series.seasons || []).length + '</td>' +
                    '<td>' + episodeCount + '</td>' +
                    '<td>' + SniparrUtils.escapeHtml(series.status || '') + '</td>' +
        ...
        _renderOverviewToContainer: function(listEl, items) {
            ...
            items.forEach(function(series) {
                ...
                div.innerHTML = '<img src="' + posterUrl + '" alt="" style="width:50px;height:75px;object-fit:cover;border-radius:6px;" onerror="this.src=\'./static/images/no-poster.png\'">' +
                    '<div><div style="font-weight:600;color:#e2e8f0;">' + SniparrUtils.escapeHtml(series.title || '') + '</div>' +
                    '<div style="font-size:0.85rem;color:#94a3b8;">' + year + (series.status ? ' · ' + SniparrUtils.escapeHtml(series.status) : '') + '</div></div>';
        ...
        _renderPosterView: function(grid, items) {
            ...
            items.forEach(function(series) {
                ...
                card.innerHTML =
                    ...
                    '<div class="media-info">' +
                        '<div class="media-title">' + SniparrUtils.escapeHtml(title) + '</div>' +
                        '<div class="media-year">' + (year || '') + (series.status ? ' &middot; ' + SniparrUtils.escapeHtml(series.status) : '') + '</div>' +
                    '</div>';
...
/**
 * Media Hunt Collection – Movie Snipe + TV Snipe instance dropdowns, combined library view.
 * Only shows Movie Snipe and TV Snipe instances (no Radarr/Sonarr).
 */
(function() {
    'use strict';

    var movieSelectId = 'media-snipe-collection-movie-instance-select';
    var tvSelectId = 'media-snipe-collection-tv-instance-select';
    ...
    window.MediaHuntCollection = {
        ...
        init: function() {
            ...
            var tvMatch = hash.match(/media-snipe-collection\/tv\/(\d+)/);
            ...
            window.TVHuntCollection._prefix = 'media-snipe-collection';
            window.MovieHuntCollection._prefix = 'media-snipe-collection';
            ...
            // Populate dropdowns from Movie Snipe, TV Snipe, and indexers (for step-2 warning) — cache-bust for fresh data on navigate
            var ts = '?_=' + Date.now();
            var moviePromise = fetch('./api/movie-snipe/instances' + ts, { cache: 'no-store' }).then(function(r) { return r.json(); }).then(function(d) { return d.instances || []; }).catch(function() { return []; });
            var tvPromise = fetch('./api/tv-snipe/instances' + ts, { cache: 'no-store' }).then(function(r) { return r.json(); }).then(function(d) { return d.instances || []; }).catch(function() { return []; });
            var indexerPromise = fetch('./api/indexer-snipe/indexers' + ts, { cache: 'no-store' }).then(function(r) { return r.json(); }).then(function(d) { return d.indexers || []; }).catch(function() { return []; });
            var hasClientsPromise = fetch('./api/movie-snipe/has-clients' + ts, { cache: 'no-store' }).then(function(r) { return r.json(); }).then(function(d) { return d.has_clients === true; }).catch(function() { return false; });
            ...
            Promise.all([moviePromise, tvPromise, indexerPromise, hasClientsPromise]).then(function(results) {
                ...
                var contentWrapper = document.getElementById('media-snipe-collection-content-wrapper');
                ...
                movieSelect.appendChild(document.createElement('option')).value = ''; movieSelect.options[0].textContent = 'No Movie Snipe instance';
                ...
                tvSelect.innerHTML = '';
                tvSelect.appendChild(document.createElement('option')).value = ''; tvSelect.options[0].textContent = 'No TV Snipe instance';
            ...
            var onInstanceChange = function() {
                ...
                if (self._movieInstanceId) {
                    fetch('./api/movie-snipe/instances/current', {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
        ...
        loadCombinedCollection: function(append) {
            var self = this;
            var grid = document.getElementById('media-snipe-collection-grid');
            if (!grid) return;
            ...
            if (!self._movieInstanceId && !self._tvInstanceId) {
                grid.innerHTML = '<div class="media-snipe-collection-no-instances">' +
                    '<i class="fas fa-cube" aria-hidden="true"></i>' +
                    '<p class="no-instances-title">No instances selected</p>' +
                    '<p class="no-instances-desc">Select a Movie Snipe and/or TV Snipe instance above to view your combined library.</p>' +
                    '<a href="./#media-snipe-instances" class="no-instances-action-btn">' +
            ...
            function ensureTvConfig() {
                ...
                return fetch('./api/tv-snipe/settings/tv-management?instance_id=' + self._tvInstanceId)
                    .then(function(r) { return r.json(); })
            ...
            function fallbackToLegacyApis() {
                ...
                if (self._movieInstanceId) {
                    promises.push(fetch('./api/movie-snipe/collection?instance_id=' + self._movieInstanceId + '&page=1&page_size=9999&sort=' + encodeURIComponent(self.sortBy || 'title.asc'))
                        .then(function(r) { return r.json(); })
                        .then(function(d) {
                ...
                if (self._tvInstanceId) {
                    promises.push(fetch('./api/tv-snipe/collection?instance_id=' + self._tvInstanceId)
                        .then(function(r) { return r.json(); })
                        .then(function(d) {
                            var series = d.series || [];
        ...
        setupCollectionInfiniteScroll: function() {
            var self = this;
            var sentinel = document.getElementById('media-snipe-collection-scroll-sentinel');
        ...
        setupSort: function() {
            var self = this;
            var select = document.getElementById('media-snipe-collection-sort');
            if (!select) return;
            var opts = [
            ...
            var saved = SniparrUtils.getUIPreference('media-snipe-collection-sort', 'title.asc');
            if (saved) select.value = saved;
            self.sortBy = select.value || 'title.asc';
            select.onchange = function() {
                self.sortBy = select.value;
                SniparrUtils.setUIPreference('media-snipe-collection-sort', self.sortBy);
                self.loadCombinedCollection();
            };
        },
        ...
        setupViewMode: function() {
            var self = this;
            var select = document.getElementById('media-snipe-collection-view-mode');
            if (!select) return;
            self.viewMode = SniparrUtils.getUIPreference('media-snipe-collection-view', 'posters') || 'posters';
            select.value = self.viewMode;
            select.onchange = function() {
                self.viewMode = select.value;
                SniparrUtils.setUIPreference('media-snipe-collection-view', self.viewMode);
                self.renderCombined();
            };
        },
        ...
        setupSearch: function() {
            var self = this;
            var input = document.getElementById('media-snipe-collection-search-input');
            if (!input) return;
            input.value = '';
            input.addEventListener('input', function() {
                ...
                self._searchTm = setTimeout(function() {
                    if (!q) {
                        document.getElementById('media-snipe-collection-search-results-view').style.display = 'none';
                        document.getElementById('media-snipe-collection-main-content').style.display = 'block';
                        self.renderCombined();
                        return;
                    }
        ...
        performSearch: function(query) {
            ...
            var grid = document.getElementById('media-snipe-collection-search-results-grid');
            var resultsView = document.getElementById('media-snipe-collection-search-results-view');
            var mainContent = document.getElementById('media-snipe-collection-main-content');
            if (!grid) return;
            resultsView.style.display = 'block';
        ...
        renderCombined: function() {
            var self = this;
            var grid = document.getElementById('media-snipe-collection-grid');
            var table = document.getElementById('media-snipe-collection-table');
            var tableBody = document.getElementById('media-snipe-collection-table-body');
            var overview = document.getElementById('media-snipe-collection-overview');
            var overviewList = document.getElementById('media-snipe-collection-overview-list');
            if (!grid) return;
        ...
        showMainView: function() {
            var r = document.getElementById('media-snipe-collection-search-results-view');
            var m = document.getElementById('media-snipe-collection-main-content');
            var d = document.getElementById('media-snipe-collection-series-detail-view');
...
/**
 * Media Hunt Calendar – Movie Snipe, Radarr, TV Snipe, Sonarr.
...
(function() {
    ...
    function getInstanceValue() {
        var sel = document.getElementById('media-snipe-calendar-instance-select');
    ...
    function loadCollectionCalendar() {
        var container = document.getElementById('media-snipe-calendar-timeline');
        if (!container) return;
        ...
        if (p.appType === 'movie_hunt' && p.instance) {
            url = './api/movie-snipe/calendar?days_past=' + pastDays + '&days_future=120&instance_id=' + encodeURIComponent(p.instance);
            ...
            container.innerHTML = '<div class="mh-cal-empty"><i class="fas fa-calendar-times"></i><p>Select a Movie Snipe or Radarr instance.</p></div>';
            return;
        }
    ...
    function loadUpcomingCalendar() {
        var container = document.getElementById('media-snipe-calendar-upcoming-timeline');
        if (!container) return;
        container.innerHTML = '<div class="loading-spinner"><i class="fas fa-spinner fa-spin"></i><p>Loading upcoming movies...</p></div>';

        fetch('./api/movie-snipe/calendar/upcoming?page=1')
            .then(function(r) { return r.json(); })
            .then(function(data) {
    ...
    function loadTVCalendar() {
        ...
        if (!p.appType || !p.instance) {
            container.innerHTML = '<div class="mh-cal-empty"><i class="fas fa-calendar-times"></i><p>Select a TV Snipe or Sonarr instance to view the calendar.</p></div>';
            return;
        }
        ...
        fetch('./api/tv-snipe/collection?instance_id=' + p.instance)
            .then(function(r) { return r.json(); })
            .then(function(data) {
    ...
    function switchTab(tab) {
        _currentTab = tab;
        var collView = document.getElementById('media-snipe-calendar-collection-view');
        var upView = document.getElementById('media-snipe-calendar-upcoming-view');
    ...
    function updateUIForMode(mode) {
        var titleEl = document.getElementById('media-snipe-calendar-title');
        var tabsWrap = document.getElementById('media-snipe-calendar-tabs-wrap');
        var legendEl = document.getElementById('media-snipe-calendar-legend');
        var upcomingView = document.getElementById('media-snipe-calendar-upcoming-view');
    ...
    function populateInstanceDropdown() {
        ...
        Promise.all([
            ...
            safeJsonFetch('./api/indexer-snipe/indexers?t=' + ts, { indexers: [] }),
            safeJsonFetch('./api/movie-snipe/has-clients?t=' + ts, { has_clients: false })
        ]).then(function(results) {
            ...
            mh.forEach(function(inst) {
                ...
                var opt = document.createElement('option');
                opt.value = v;
                opt.textContent = 'Movie Snipe \u2013 ' + (inst.name || inst.id);
                sel.appendChild(opt);
            ...
            tvh.forEach(function(inst) {
                ...
                var opt = document.createElement('option');
                opt.value = v;
                opt.textContent = 'TV Snipe \u2013 ' + (inst.name || inst.id);
                sel.appendChild(opt);
            ...
            if (sel.options.length === 0) {
                ...
                empty.value = '';
                empty.textContent = 'No Movie or TV Snipe instances';
                sel.appendChild(empty);
                _collectionLoaded = false;
                _upcomingLoaded = false;
                var timeline = document.getElementById('media-snipe-calendar-timeline');
                var upcomingTimeline = document.getElementById('media-snipe-calendar-upcoming-timeline');
                if (timeline) timeline.innerHTML = '<div class="mh-cal-empty"><i class="fas fa-calendar-times"></i><p>Configure a Movie Snipe or TV Snipe instance to view the calendar.</p></div>';
                if (upcomingTimeline) upcomingTimeline.innerHTML = '<div class="mh-cal-empty"><i class="fas fa-calendar-times"></i><p>Configure a Movie Snipe instance to discover upcoming releases.</p></div>';
                return;
            }
            if (preferred) {
                sel.value = preferred;
    ...
    document.addEventListener('sniparr:tv-snipe-instances-changed', function() { populateInstanceDropdown(); });
 ...
 * File: media-snipe-custom-formats.js. Uses /api/tv-snipe/ endpoints; DOM IDs remain tv-snipe-* for compatibility.
 */
(function() {
    'use strict';
    ...
    window.TVHuntCustomFormats = {
        ...
        refreshList: function() {
            if (window.TVHuntInstanceDropdown && document.getElementById('tv-snipe-settings-custom-formats-instance-select') && !window.TVHuntCustomFormats._instanceDropdownAttached) {
                window.TVHuntInstanceDropdown.attach('tv-snipe-settings-custom-formats-instance-select', function() { window.TVHuntCustomFormats.refreshList(); });
                window.TVHuntCustomFormats._instanceDropdownAttached = true;
            }
            var preformattedGrid = document.getElementById('tv-snipe-custom-formats-preformatted-grid');
            var importedGrid = document.getElementById('tv-snipe-custom-formats-imported-grid');
            if (!preformattedGrid || !importedGrid) return;
            
            fetch('./api/tv-snipe/custom-formats')
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    ...
                    for (var g = 0; g < sortedGroups.length; g++) {
                        ...
                        for (var j = 0; j < groupItems.length; j++) {
                            ...
                            preformattedHtml += '<div class="custom-format-card instance-card" data-index="' + idx + '" data-app-type="tv-snipe-custom-format">' +
                    ...
                    for (var k = 0; k < importedItems.length; k++) {
                        ...
                        importedHtml += '<div class="custom-format-card instance-card" data-index="' + idx + '" data-app-type="tv-snipe-custom-format">' +
                            '<div class="custom-format-card-header">' +
                            '<div class="custom-format-card-title"><i class="fas fa-code"></i><span>' + title + '</span></div>' +
                            '</div>' +
                            '<div class="custom-format-card-footer">' +
                    ...
                    var deletePreBtn = document.getElementById('tv-snipe-delete-all-preformatted');
                    var deleteImpBtn = document.getElementById('tv-snipe-delete-all-imported');
        ...
        _bindAddButtons: function() {
            var addPreformattedBtn = document.getElementById('tv-snipe-add-preformatted-btn');
            var addImportedBtn = document.getElementById('tv-snipe-add-imported-btn');
        ...
        openViewModal: function(index) {
            ...
            document.getElementById('tv-snipe-custom-format-view-modal-title').textContent = 'View JSON: ' + title;
            var jsonStr = item.custom_format_json || '{}';
            try {
                var parsed = JSON.parse(jsonStr);
                jsonStr = JSON.stringify(parsed, null, 2);
            } catch (e) { /* show as-is */ }
            document.getElementById('tv-snipe-custom-format-view-json').textContent = jsonStr;
            document.getElementById('tv-snipe-custom-format-view-modal').style.display = 'flex';
            document.body.classList.add('custom-format-modal-open');
        },

        closeViewModal: function() {
            var modal = document.getElementById('tv-snipe-custom-format-view-modal');
            if (modal) modal.style.display = 'none';
        ...
        _ensureAddModalInBody: function() {
            var modal = document.getElementById('tv-snipe-custom-format-modal');
        ...
        openAddModal: function(source) {
            ...
            if (source === 'preformat') {
                document.getElementById('tv-snipe-custom-format-modal-title').textContent = 'Add Pre-Formatted';
                document.getElementById('tv-snipe-custom-format-preformat-area').style.display = 'block';
                var importArea = document.getElementById('tv-snipe-custom-format-import-area');
                if (importArea) importArea.style.display = 'none';
                window.TVHuntCustomFormats._loadPreformatTree();
            } else {
                document.getElementById('tv-snipe-custom-format-modal-title').textContent = 'Add Imported';
                document.getElementById('tv-snipe-custom-format-preformat-area').style.display = 'none';
                var importArea = document.getElementById('tv-snipe-custom-format-import-area');
                if (importArea) importArea.style.display = 'block';
            }

            document.getElementById('tv-snipe-custom-format-modal-save').innerHTML = '<i class="fas fa-plus"></i> Add';
            document.getElementById('tv-snipe-custom-format-json-textarea').value = '';
            document.getElementById('tv-snipe-custom-format-modal').style.display = 'flex';
            document.body.classList.add('custom-format-modal-open');
        },
        ...
        openEditModal: function(index) {
            ...
            var item = list[index];
            document.getElementById('tv-snipe-custom-format-modal-title').textContent = 'Edit Custom Format';
            document.getElementById('tv-snipe-custom-format-modal-save').innerHTML = '<i class="fas fa-save"></i> Save';
            document.getElementById('tv-snipe-custom-format-source-import').checked = true;
            document.getElementById('tv-snipe-custom-format-preformat-area').style.display = 'none';
            var importArea = document.getElementById('tv-snipe-custom-format-import-area');
            if (importArea) importArea.style.display = 'block';
            document.getElementById('tv-snipe-custom-format-json-textarea').value = item.custom_format_json || '{}';
            document.getElementById('tv-snipe-custom-format-modal').style.display = 'flex';
            document.body.classList.add('custom-format-modal-open');
        },
        ...
        _loadPreformatTree: function() {
            var treeEl = document.getElementById('tv-snipe-custom-format-preformat-tree');
            if (!treeEl) return;
            ...
            fetch('./api/tv-snipe/custom-formats/preformats')
                .then(function(r) { return r.json(); })
                .then(function(data) {
        ...
        _onSourceChange: function() {
            var isPre = document.getElementById('tv-snipe-custom-format-source-preformat').checked;
            var preformatArea = document.getElementById('tv-snipe-custom-format-preformat-area');
            var importArea = document.getElementById('tv-snipe-custom-format-import-area');
            var jsonTa = document.getElementById('tv-snipe-custom-format-json-textarea');
        ...
        saveModal: function() {
            ...
            if (editing != null) {
                var jsonRaw = document.getElementById('tv-snipe-custom-format-json-textarea').value.trim();
                if (!jsonRaw) {
                    if (window.sniparrUI && window.sniparrUI.showNotification) {
                ...
                fetch('./api/tv-snipe/custom-formats/' + editing, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
            ...
            if (isPre) {
                var tree = document.getElementById('tv-snipe-custom-format-preformat-tree');
        ...
        deleteFormat: function(index) {
            var doDelete = function() {
                fetch('./api/tv-snipe/custom-formats/' + index, { method: 'DELETE' })
                    .then(function(r) { return r.json(); })
                    .then(function(data) {
                        if (data.success) {
                            if (window.sniparrUI && window.sniparrUI.showNotification) {
        ...
        deleteAllByType: function(type) {
            ...
            function runDeleteAll() {
                ...
                function deleteNext() {
                    ...
                    fetch('./api/tv-snipe/custom-formats/' + idx, { method: 'DELETE' })
                        .then(function(r) { return r.json(); })
                        .then(function(data) {
        ...
        init: function() {
            ...
            var modal = document.getElementById('tv-snipe-custom-format-modal');
            var backdrop = document.getElementById('tv-snipe-custom-format-modal-backdrop');
            var closeBtn = document.getElementById('tv-snipe-custom-format-modal-close');
            var cancelBtn = document.getElementById('tv-snipe-custom-format-modal-cancel');
            var saveBtn = document.getElementById('tv-snipe-custom-format-modal-save');
            ...
            var viewModal = document.getElementById('tv-snipe-custom-format-view-modal');
            var viewBackdrop = document.getElementById('tv-snipe-custom-format-view-modal-backdrop');
            var viewCloseBtn = document.getElementById('tv-snipe-custom-format-view-modal-close');
            var viewCloseBtnFooter = document.getElementById('tv-snipe-custom-format-view-modal-close-btn');
            ...
            var deleteAllPreBtn = document.getElementById('tv-snipe-delete-all-preformatted');
            var deleteAllImpBtn = document.getElementById('tv-snipe-delete-all-imported');
            if (deleteAllPreBtn) {
            ...
            document.querySelectorAll('input[name="tv-snipe-custom-format-source"]').forEach(function(radio) {
...
/**
 * Media Hunt – Root Folders for TV. File: media-snipe-root-folders.js.
 * Card grid, add via modal, browse, test, set default, delete. Uses /api/tv-snipe/root-folders; DOM IDs remain tv-snipe-*.
 */
(function() {
    'use strict';
    ...
    window.TVHuntRootFolders = {
        ...
        refreshList: function() {
            if (window.TVHuntInstanceDropdown && document.getElementById('tv-snipe-settings-root-folders-instance-select') && !window.TVHuntRootFolders._instanceDropdownAttached) {
                window.TVHuntInstanceDropdown.attach('tv-snipe-settings-root-folders-instance-select', function() {
            ...
            var gridEl = document.getElementById('tv-snipe-root-folders-grid');
            if (!gridEl) return;
            fetch('./api/tv-snipe/root-folders', { cache: 'no-store' })
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    ...
                    for (var i = 0; i < folders.length; i++) {
                        ...
                        html += '<div class="root-folder-card instance-card' + defaultClass + '" data-index="' + idx + '" data-app-type="tv-snipe-root-folder">' +
                    ...
                    html += '<div class="add-instance-card add-root-folder-card" id="tv-snipe-root-folders-add-card" data-app-type="tv-snipe-root-folder">' +
                ...
                .catch(function() {
                    var addCard = '<div class="add-instance-card add-root-folder-card" id="tv-snipe-root-folders-add-card" data-app-type="tv-snipe-root-folder">' +
                        '<div class="add-icon"><i class="fas fa-plus-circle"></i></div>' +
                        '<div class="add-text">Add Root Folder</div></div>';
                    gridEl.innerHTML = '<p style="color: #ef4444; margin: 0 0 12px 0;">Failed to load TV Snipe root folders.</p>' + addCard;
        ...
        _bindAddCard: function() {
            var addCard = document.getElementById('tv-snipe-root-folders-add-card');
            if (addCard) {
        ...
        openAddModal: function() {
            var modal = document.getElementById('tv-snipe-root-folder-add-modal');
            var input = document.getElementById('tv-snipe-root-folder-add-path');
            if (modal && modal.parentNode !== document.body) {
                document.body.appendChild(modal);
            }
            ...
            document.body.classList.add('tv-snipe-root-folder-add-modal-open');
        },

        closeAddModal: function() {
            var modal = document.getElementById('tv-snipe-root-folder-add-modal');
            if (modal) modal.style.display = 'none';
            document.body.classList.remove('tv-snipe-root-folder-add-modal-open');
        },

        setDefault: function(index) {
            if (typeof index !== 'number' || index < 0) return;
            fetch('./api/tv-snipe/root-folders/' + index + '/default', { method: 'PATCH' })
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    if (data.success) {
                        if (window.sniparrUI && window.sniparrUI.showNotification) {
                            window.sniparrUI.showNotification('Default TV Snipe root folder updated.', 'success');
                        }
        ...
        testPath: function(path) {
            if (!path || (typeof path !== 'string')) {
                var addInput = document.getElementById('tv-snipe-root-folder-add-path');
            ...
            var testBtn = document.getElementById('tv-snipe-root-folder-add-test-btn');
            if (testBtn) {
                testBtn.disabled = true;
                testBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Testing...';
            }
            fetch('./api/tv-snipe/root-folders/test', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        addFolder: function() {
            ...
            var saveBtn = document.getElementById('tv-snipe-root-folder-add-modal-save');
            if (saveBtn) {
                saveBtn.disabled = true;
                saveBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Adding...';
            }
            fetch('./api/tv-snipe/root-folders', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ path: path })
            })
                ...
                .then(function(result) {
                    ...
                    if (result.ok && result.data && result.data.success) {
                        ...
                        if (window.sniparrUI && window.sniparrUI.showNotification) {
                            window.sniparrUI.showNotification('TV Snipe root folder added.', 'success');
                        }
                        window.TVHuntRootFolders.refreshList();
                    } else {
        ...
        deleteFolder: function(index) {
            if (typeof index !== 'number' || index < 0) return;
            var deleteUrl = './api/tv-snipe/root-folders/' + index;
            var doDelete = function() {
                fetch(deleteUrl, { method: 'DELETE' })
                    ...
                    .then(function(data) {
                        if (data.success) {
                            if (window.sniparrUI && window.sniparrUI.showNotification) {
                                window.sniparrUI.showNotification('TV Snipe root folder removed.', 'success');
                            }
                            window.TVHuntRootFolders.refreshList();
                        } else {
                            if (window.sniparrUI && window.sniparrUI.showNotification) {
            ...
            if (window.SniparrConfirm && window.SniparrConfirm.show) {
                window.SniparrConfirm.show({
                    ...
                    message: 'Remove this TV Snipe root folder?',
                    confirmLabel: 'OK',
                    onConfirm: doDelete
                });
            } else {
                if (!confirm('Remove this TV Snipe root folder?')) return;
                doDelete();
            }
        },

        openBrowseModal: function(sourceInput) {
            var modal = document.getElementById('tv-snipe-root-folders-browse-modal');
            var browsePathInput = document.getElementById('tv-snipe-root-folders-browse-path-input');
            window.TVHuntRootFolders._browseTargetInput = sourceInput || document.getElementById('tv-snipe-root-folder-add-path');
            ...
            modal.style.display = 'flex';
            document.body.classList.add('tv-snipe-root-folders-browse-modal-open');
        ...
        closeBrowseModal: function() {
            ...
            if (modal) {
                modal.style.display = 'none';
                document.body.classList.remove('tv-snipe-root-folders-browse-modal-open');
            }
        },

        confirmBrowseSelection: function() {
            var pathInput = document.getElementById('tv-snipe-root-folders-browse-path-input');
            var target = window.TVHuntRootFolders._browseTargetInput || document.getElementById('tv-snipe-root-folder-add-path');
            if (pathInput && target) {
        ...
        browseCreateFolder: function() {
            ...
            name = (name || '').trim();
            if (!name) return;
            fetch('./api/tv-snipe/root-folders/browse/create', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        browseRenameFolder: function(path, currentName) {
            ...
            fetch('./api/tv-snipe/root-folders/browse/rename', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        browseDeleteFolder: function(path) {
            ...
            fetch('./api/tv-snipe/root-folders/browse/delete', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ path: path })
            }).then(function(r) { return r.json(); }).then(function(data) {
        ...
        loadBrowsePath: function(path) {
            var listEl = document.getElementById('tv-snipe-root-folders-browse-list');
            var pathInput = document.getElementById('tv-snipe-root-folders-browse-path-input');
            var upBtn = document.getElementById('tv-snipe-root-folders-browse-up');
            ...
            fetch('./api/tv-snipe/root-folders/browse?path=' + encodeURIComponent(path))
                .then(function(r) { return r.json(); })
                .then(function(data) {
        ...
        init: function() {
            ...
            var addBackdrop = document.getElementById('tv-snipe-root-folder-add-modal-backdrop');
            var addClose = document.getElementById('tv-snipe-root-folder-add-modal-close');
            var addCancel = document.getElementById('tv-snipe-root-folder-add-modal-cancel');
            var addSave = document.getElementById('tv-snipe-root-folder-add-modal-save');
            var addBrowseBtn = document.getElementById('tv-snipe-root-folder-add-browse-btn');
            var addTestBtn = document.getElementById('tv-snipe-root-folder-add-test-btn');
            var addPathInput = document.getElementById('tv-snipe-root-folder-add-path');
            ...
            document.addEventListener('keydown', function(e) {
                if (e.key === 'Escape') {
                    if (document.getElementById('tv-snipe-root-folder-add-modal') && document.getElementById('tv-snipe-root-folder-add-modal').style.display === 'flex') {
                        self.closeAddModal();
                    }
                    if (document.getElementById('tv-snipe-root-folders-browse-modal') && document.getElementById('tv-snipe-root-folders-browse-modal').style.display === 'flex') {
                        self.closeBrowseModal();
                    }
                }
            });
            var browseBackdrop = document.getElementById('tv-snipe-root-folders-browse-backdrop');
            var browseClose = document.getElementById('tv-snipe-root-folders-browse-close');
            var browseCancel = document.getElementById('tv-snipe-root-folders-browse-cancel');
            var browseOk = document.getElementById('tv-snipe-root-folders-browse-ok');
            var browsePathInput = document.getElementById('tv-snipe-root-folders-browse-path-input');
            ...
            var newFolderBtn = document.getElementById('tv-snipe-root-folders-browse-new-folder');
