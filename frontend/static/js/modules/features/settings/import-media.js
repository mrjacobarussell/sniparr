/**
 * Import Media — Unmapped folder import system for Movie Snipe.
...
(function() {
    ...
    window.ImportMedia = {
        ...
        loadItems: function() {
            ...
            fetch('./api/movie-snipe/import-media?' + self.getInstanceParam().replace('&', ''))
        ...
        triggerScan: function() {
            ...
            fetch('./api/movie-snipe/import-media/scan?' + self.getInstanceParam().replace('&', ''), {
                method: 'POST'
            })
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    if (data.success) {
                        ...
                        if (window.sniparrUI && window.sniparrUI.showNotification) {
                            window.sniparrUI.showNotification(data.message || 'Scan failed', 'error');
                        }
                        if (btn) {
                            btn.disabled = false;
        ...
        confirmItem: function(folderPath) {
            ...
            var m = item.best_match;
            fetch('./api/movie-snipe/import-media/confirm?' + self.getInstanceParam().replace('&', ''), {
                method: 'POST',
                ...
                .then(function(data) {
                    if (data.success) {
                        if (window.sniparrUI && window.sniparrUI.showNotification) {
                            window.sniparrUI.showNotification(data.message || 'Imported!', 'success');
                        }
                        self.loadItems();
                    } else {
                        if (window.sniparrUI && window.sniparrUI.showNotification) {
                            window.sniparrUI.showNotification(data.message || 'Import failed', data.already_exists ? 'info' : 'error');
                        }
                ...
                .catch(function(err) {
                    ...
                    if (window.sniparrUI && window.sniparrUI.showNotification) {
                        window.sniparrUI.showNotification('Import failed', 'error');
                    }
                });
        },
        ...
        _doImportAll: function() {
            var self = this;
            fetch('./api/movie-snipe/import-media/confirm-all?' + self.getInstanceParam().replace('&', ''), {
                method: 'POST'
            })
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    if (data.success) {
                        if (window.sniparrUI && window.sniparrUI.showNotification) {
                            window.sniparrUI.showNotification(data.message || 'All imported!', 'success');
                        }
                    } else {
                        if (window.sniparrUI && window.sniparrUI.showNotification) {
                            window.sniparrUI.showNotification(data.message || 'Import failed', 'error');
                        }
                    }
                    self.loadItems();
                })
        ...
        skipItem: function(folderPath) {
            var self = this;
            fetch('./api/movie-snipe/import-media/skip?' + self.getInstanceParam().replace('&', ''), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        performManualSearch: function() {
            ...
            var url = './api/movie-snipe/import-media/search?q=' + encodeURIComponent(query);
