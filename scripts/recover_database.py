#!/usr/bin/env python3
"""
Sniparr Database Recovery Utility
...
Usage:
- For Docker: docker exec sniparr python /app/scripts/recover_database.py
...
def get_database_path():
    ...
    if windows_config:
        return Path(windows_config) / "sniparr.db"
    
    # Check for Windows AppData
    import platform
    if platform.system() == "Windows":
        appdata = os.environ.get("APPDATA", os.path.expanduser("~"))
        return Path(appdata) / "Sniparr" / "sniparr.db"
    
    # Local development
    project_root = Path(__file__).parent.parent
    return project_root / "data" / "sniparr.db"
...
def backup_corrupted_database(db_path):
    ...
    backup_path = db_path.parent / f"sniparr_corrupted_backup_{timestamp}.db"
...
def create_fresh_database(db_path):
    ...
    try:
        ...
        from primary.utils.database import SniparrDatabase
...
def main():
    print("🔥 Sniparr Database Recovery Utility")
    print("=" * 50)
